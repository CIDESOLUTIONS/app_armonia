...
export function usePQR({
  ...
  const createPQR = useCallback(
    async (pqrData: Partial<PQR>): Promise<PQR> => {
      try {
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error creating PQR";
  ...
  const updatePQR = useCallback(
    async (id: number, updates: Partial<PQR>): Promise<PQR> => {
      try {
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error updating PQR";
        setError(errorMessage);
        throw err;
      }
    },
  ...
  const deletePQR = useCallback(async (id: number): Promise<void> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Error deleting PQR";
      setError(errorMessage);
      throw err;
    }
  }, []);
