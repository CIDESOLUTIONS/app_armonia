...
export function useCameras(): UseCamerasReturn {
  ...
  const discoverCameras = useCallback(
    async (timeout: number = 30000): Promise<Camera[]> => {
      try {
        setLoading(true);
        setError(null);

        const response = await apiClient.post("/cameras/discovery", {
          timeout,
        });
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error descubriendo cámaras";
  ...
  const registerCamera = useCallback(
    async (data: RegisterCameraData): Promise<boolean> => {
      try {
        setLoading(true);
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error registrando cámara";
        setError(errorMessage);
  ...
  const updateCamera = useCallback(
    async (
      ...
      try {
        ...
        const response = await apiClient.put("/cameras/manage", {
          id,
          ...updates,
        });
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error actualizando cámara";
        setError(errorMessage);
  ...
  const deleteCamera = useCallback(
    async (id: number): Promise<boolean> => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error eliminando cámara";
        setError(errorMessage);
  ...
  const loadCameras = useCallback(async (): Promise<void> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Error cargando cámaras";
      setError(errorMessage);
  ...
  const getStreamUrl = useCallback(
    async (cameraId: number, profileToken?: string): Promise<string | null> => {
      try {
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error obteniendo stream";
        setError(errorMessage);
  ...
  const getStreamProfiles = useCallback(
    async (cameraId: number): Promise<StreamProfile[]> => {
      try {
        setError(null);
  ...
  const captureSnapshot = useCallback(
    async (cameraId: number): Promise<string | null> => {
      try {
        setError(null);

        // Hacer request para snapshot
        const response = await fetch(
          `/api/cameras/stream?cameraId=${cameraId}`,
          {
            method: "GET",
            headers: {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error capturando imagen";
        setError(errorMessage);
  ...
  const movePTZ = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error moviendo cámara PTZ";
        setError(errorMessage);
  ...
  const stopPTZ = useCallback(async (cameraId: number): Promise<boolean> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Error deteniendo PTZ";
      setError(errorMessage);
  ...
  const gotoPreset = useCallback(
    async (cameraId: number, presetToken: string): Promise<boolean> => {
      try {
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error yendo a preset";
        setError(errorMessage);
  ...
  const getPTZCapabilities = useCallback(
    async (cameraId: number): Promise<PTZCapabilities | null> => {
      try {
        setError(null);

        const response = await apiClient.get(
          `/cameras/ptz?cameraId=${cameraId}`,
        );
  ...
  const checkCameraStatus = useCallback(
    async (cameraId: number): Promise<"ONLINE" | "OFFLINE" | "ERROR"> => {
      try {
        // Simular verificación de estado
        const statuses: Array<"ONLINE" | "OFFLINE" | "ERROR"> = [
          "ONLINE",
          "OFFLINE",
          "ERROR",
        ];
