...
describe("PQR System Integration Tests - Flujos Principales", () => {
  ...
  describe("Flujo completo de PQR con asignación inteligente", () => {
    it("debe procesar un PQR desde creación hasta resolución con asignación inteligente", async () => {
      ...
      const assignNotificationResult =
      ...
      const inProgressNotificationResult =
        await notificationService.notifyStatusChange(
          mockPQR.id,
      ...
      const resolvedPQR = {
        ...
        resolvedAt: new Date(),
        resolution:
          "Se reparó la fuga de agua y se verificó su correcto funcionamiento",
      };
      ...
      const resolvedNotificationResult =
        await notificationService.notifyStatusChange(
          mockPQR.id,
          PQRStatus.RESOLVED,
          PQRStatus.IN_PROGRESS,
      ...
      const surveyResult = await notificationService.sendSatisfactionSurvey(
        mockPQR.id,
      );
      ...
      const closedNotificationResult =
        await notificationService.notifyStatusChange(
          mockPQR.id,
          PQRStatus.CLOSED,
          PQRStatus.RESOLVED,
  ...
  describe("Flujo de PQR con reapertura y escalamiento", () => {
    it("debe manejar correctamente la reapertura y escalamiento de un PQR", async () => {
      ...
      const reopenNotificationResult =
        await notificationService.notifyStatusChange(
          reopenedPQR.id,
          PQRStatus.REOPENED,
          PQRStatus.RESOLVED,
      ...
      const escalationNotificationResult =
        await notificationService.notifyStatusChange(
          escalatedPQR.id,
          PQRStatus.ASSIGNED,
          PQRStatus.REOPENED,
      ...
      const specialistAssignNotificationResult =
        await notificationService.notifyStatusChange(
      ...
      const finalResolvedPQR = {
        ...
        resolvedAt: new Date(),
        resolution:
          "Se identificó y reparó un problema en el tablero principal que causaba los cortes intermitentes",
      };
      ...
      const finalResolvedNotificationResult =
        await notificationService.notifyStatusChange(
          finalResolvedPQR.id,
          PQRStatus.RESOLVED,
          PQRStatus.ASSIGNED,
  ...
  describe("Generación de dashboard completo", () => {
    it("debe generar dashboard completo con todos los indicadores", async () => {
      ...
      expect(metricsService.getSummaryMetrics).toHaveBeenLastCalledWith(
        categoryFilter,
      );
      expect(metricsService.getCategoryDistribution).toHaveBeenLastCalledWith(
        categoryFilter,
      );
