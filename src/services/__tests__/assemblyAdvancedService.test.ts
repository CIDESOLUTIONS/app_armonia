...
describe("AssemblyAdvancedService", () => {
  ...
  describe("registerAttendance", () => {
    it("debe registrar asistencia y actualizar el quórum", async () => {
      ...
      // Ejecutar el método
      const result = await service.registerAttendance(
        assemblyId,
        attendanceData,
      );
    ...
    it("debe rechazar registro si la unidad ya está registrada", async () => {
      ...
      prisma.assembly.findUnique.mockResolvedValue(mockAssembly);
      prisma.assemblyAttendee.findFirst.mockResolvedValue(
        mockExistingAttendance,
      );

      // Ejecutar el método y verificar que lanza error
      await expect(
        service.registerAttendance(assemblyId, attendanceData),
      ).rejects.toThrow(
        `Ya existe un registro de asistencia para la propiedad ${attendanceData.propertyUnitId} en esta asamblea`,
      );
    });
  });
  ...
  describe("updateQuorum", () => {
    it("debe actualizar el quórum y notificar cuando se alcanza", async () => {
      ...
      // Mock de respuestas de Prisma
      const mockAttendees = [{ coefficient: 30 }, { coefficient: 45 }];
  ...
  describe("startAssembly", () => {
    ...
    it("debe rechazar inicio si no se ha alcanzado el quórum", async () => {
      ...
      // Ejecutar el método y verificar que lanza error
      await expect(service.startAssembly(assemblyId, userId)).rejects.toThrow(
  ...
  describe("createVoting", () => {
    ...
    it("debe rechazar creación si la asamblea no está en progreso", async () => {
      ...
      // Ejecutar el método y verificar que lanza error
      await expect(service.createVoting(votingData, userId)).rejects.toThrow(
        `No se puede crear una votación en una asamblea con estado ${mockAssembly.status}`,
      );
    });
  });
  ...
  describe("castVote", () => {
    ...
    it("debe rechazar voto si ya ha votado", async () => {
      ...
      // Ejecutar el método y verificar que lanza error
      await expect(service.castVote(voteData, userId)).rejects.toThrow(
        "Ya ha emitido un voto en esta votación",
      );
    });
  });
  ...
  describe("endAssembly", () => {
    ...
    it("debe rechazar finalización si hay votaciones activas", async () => {
      ...
      // Ejecutar el método y verificar que lanza error
      await expect(service.endAssembly(assemblyId, userId)).rejects.toThrow(
