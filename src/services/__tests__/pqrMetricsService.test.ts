...
describe("PQRMetricsService", () => {
  ...
  describe("generateDashboardMetrics", () => {
    it("debe generar métricas completas para el dashboard", async () => {
      ...
      jest
        .spyOn(service, "getCategoryDistribution")
        .mockResolvedValueOnce([
          { category: "MAINTENANCE", count: 15, percentage: 53.57 },
        ]);

      jest
        .spyOn(service, "getPriorityDistribution")
        .mockResolvedValueOnce([
          { priority: "HIGH", count: 10, percentage: 35.71 },
        ]);

      jest
        .spyOn(service, "getStatusDistribution")
        .mockResolvedValueOnce([
      ...
      expect(service.getCategoryDistribution).toHaveBeenLastCalledWith(
        testFilter,
      );
      expect(service.getPriorityDistribution).toHaveBeenLastCalledWith(
        testFilter,
      );
      expect(service.getStatusDistribution).toHaveBeenLastCalledWith(
        testFilter,
      );
    ...
    it("debe manejar errores correctamente", async () => {
      // Forzar error
      jest
        .spyOn(service, "getSummaryMetrics")
        .mockRejectedValueOnce(new Error("Error de prueba"));

      // Ejecutar el método
      await expect(service.generateDashboardMetrics()).rejects.toThrow(
        "Error de prueba",
      );
    });
  });
