...
jest.mock("@/lib/pdf/pdfGenerator", () => ({
  generatePDF: jest
    .fn()
    .mockResolvedValue("/uploads/receipts/test-receipt.pdf"),
}));
...
describe("FinancialService", () => {
  ...
  describe("generateReceipt", () => {
    ...
    it("debe fallar si las cuotas no existen", async () => {
      ...
      jest
        .spyOn(financialService as any, "getFeeById")
  ...
  describe("sendReceiptByEmail", () => {
    it("debe enviar un recibo por correo correctamente", async () => {
      ...
      jest
        .spyOn(financialService["prisma"], "$queryRawUnsafe")
      ...
      const result = await financialService.sendReceiptByEmail(
        1,
        "test@example.com",
      );
  ...
  describe("generateBulkReceipts", () => {
    it("debe generar recibos masivamente", async () => {
      ...
      jest
        .spyOn(financialService, "generateReceipt")
        .mockResolvedValueOnce({
          id: 1,
          receiptNumber: "R-101-123456789",
