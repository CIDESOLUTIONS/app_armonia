...
export class AssemblyAdvancedService {
  ...
  async registerAttendance(
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `No se puede registrar asistencia en una asamblea con estado ${assembly.status}`,
        );
      }
      ...
      if (existingAttendance) {
        throw new Error(
          `Ya existe un registro de asistencia para la propiedad ${data.propertyUnitId} en esta asamblea`,
        );
      }
      ...
      ServerLogger.error(
        `Error al registrar asistencia para asamblea ${assemblyId}:`,
        error,
      );
      throw error;
    }
  ...
  async updateQuorum(assemblyId: number) {
    try {
      ...
      const totalCoefficient = attendees.reduce(
        (sum, attendee) => sum + attendee.coefficient,
        0,
      );
      ...
      const updatedAssembly = await prisma.assembly.update({
        ...
        data: {
          ...
          quorumReachedAt:
            quorumReached && !assembly.quorumReachedAt
              ? new Date()
              : assembly.quorumReachedAt,
        },
      });
      ...
      if (
        quorumReached &&
        (!assembly.quorumReachedAt || assembly.quorumStatus !== "REACHED")
      ) {
        const quorumPercentage = Math.round(
          (totalCoefficient / assembly.requiredCoefficient) * 100,
        );
      ...
      ServerLogger.error(
        `Error al actualizar quórum para asamblea ${assemblyId}:`,
        error,
      );
      throw error;
    }
  ...
  async startAssembly(assemblyId: number, userId: number) {
    try {
      ...
      if (assembly.status !== "SCHEDULED") {
        throw new Error(
          `No se puede iniciar una asamblea con estado ${assembly.status}`,
        );
      }
      ...
      if (assembly.quorumStatus !== "REACHED") {
        throw new Error(
          "No se puede iniciar la asamblea porque no se ha alcanzado el quórum requerido",
        );
      }
  ...
  async createVoting(
    ...
    try {
      ...
      if (assembly.status !== "IN_PROGRESS") {
        throw new Error(
          `No se puede crear una votación en una asamblea con estado ${assembly.status}`,
        );
      }
  ...
  async startVoting(votingId: number, userId: number) {
    try {
      ...
      if (voting.status !== "PENDING") {
        throw new Error(
          `No se puede iniciar una votación con estado ${voting.status}`,
        );
      }

      if (voting.assembly.status !== "IN_PROGRESS") {
        throw new Error(
          `No se puede iniciar una votación en una asamblea con estado ${voting.assembly.status}`,
        );
      }
  ...
  async castVote(
    ...
    try {
      ...
      if (voting.status !== "ACTIVE") {
        throw new Error(
          `No se puede votar en una votación con estado ${voting.status}`,
        );
      }
      ...
      if (!attendee || attendee.assemblyId !== voting.assemblyId) {
        throw new Error(
          "El usuario no está registrado como asistente en esta asamblea",
        );
      }
      ...
      ServerLogger.error(
        `Error al emitir voto en votación ${data.votingId}:`,
        error,
      );
      throw error;
    }
  ...
  private async updateVotingCounters(votingId: number) {
    try {
      ...
      const totalCoefficientVoted = votes.reduce(
      ...
      ServerLogger.error(
        `Error al actualizar contadores de votación ${votingId}:`,
        error,
      );
      throw error;
    }
  ...
  async closeVoting(votingId: number, userId: number) {
    try {
      ...
      if (voting.status !== "ACTIVE") {
        throw new Error(
          `No se puede cerrar una votación con estado ${voting.status}`,
        );
      }
      ...
      switch (voting.type) {
        case "SIMPLE_MAJORITY":
          ...
          if (counters.result["Sí"] && counters.result["No"]) {
            isApproved =
              counters.result["Sí"].count > counters.result["No"].count;
          }
          break;
        ...
        case "QUALIFIED_MAJORITY":
          ...
          if (voting.requiredPercentage && counters.result["Sí"]) {
            const baseValue =
              voting.baseForPercentage === "TOTAL_COEFFICIENTS"
                ? voting.assembly.requiredCoefficient
                : counters.totalCoefficientVoted;

            const approvalPercentage =
              (counters.result["Sí"].coefficient / baseValue) * 100;
        ...
        case "COEFFICIENT_BASED":
          ...
          if (counters.result["Sí"] && voting.requiredPercentage) {
            const approvalPercentage =
              (counters.result["Sí"].coefficient /
                counters.totalCoefficientVoted) *
              100;
            isApproved = approvalPercentage >= voting.requiredPercentage;
          }
  ...
  async endAssembly(assemblyId: number, userId: number) {
    try {
      ...
      if (assembly.status !== "IN_PROGRESS") {
        throw new Error(
          `No se puede finalizar una asamblea con estado ${assembly.status}`,
        );
      }
      ...
      if (activeVotings > 0) {
        throw new Error(
          "No se puede finalizar la asamblea mientras haya votaciones activas",
        );
      }
      ...
      await activityLogger.logActivity({
        ...
        details: {
          ...
          duration: updatedAssembly.endTime
            ? Math.round(
                (updatedAssembly.endTime.getTime() - assembly.date.getTime()) /
                  60000,
              )
            : null,
        },
  ...
  async initializeMinutes(assemblyId: number, userId: number) {
    try {
      ...
      if (assembly.status !== "COMPLETED") {
        throw new Error(
          `No se puede generar acta para una asamblea con estado ${assembly.status}`,
        );
      }
      ...
      this.generateMinutesContent(minutes.id, userId).catch((error) => {
        ServerLogger.error(
          `Error en generación asíncrona de acta ${minutes.id}:`,
          error,
        );
      });
      ...
      ServerLogger.error(
        `Error al inicializar acta para asamblea ${assemblyId}:`,
        error,
      );
      throw error;
    }
  ...
  private async generateMinutesContent(minutesId: number, userId: number) {
    try {
      ...
      ServerLogger.error(
        `Error al generar contenido de acta ${minutesId}:`,
        error,
      );
      throw error;
    }
  ...
  private async generateMinutesPdf(minutesId: number) {
    try {
      ...
      if (!minutes || !minutes.content) {
        throw new Error(
          `Acta con ID ${minutesId} no encontrada o sin contenido`,
        );
      }
  ...
  async registerRequiredSigners(
    ...
    try {
      ...
      if (minutes.status !== "GENERATED" && minutes.status !== "SIGNING") {
        throw new Error(
          `No se pueden registrar firmantes para un acta con estado ${minutes.status}`,
        );
      }
      ...
      ServerLogger.error(
        `Error al registrar firmantes para acta ${minutesId}:`,
        error,
      );
      throw error;
    }
  ...
  async signMinutes(
    ...
    try {
      ...
      if (signature.status !== "PENDING") {
        throw new Error(
          `No se puede firmar un documento con estado de firma ${signature.status}`,
        );
      }
  ...
  private async finalizeSignedMinutes(minutesId: number, userId: number) {
    try {
      ...
      const pendingSignatures = minutes.signatures.filter(
        (s) => s.status !== "SIGNED",
      );

      if (pendingSignatures.length > 0) {
        throw new Error(
          "No se puede finalizar el acta porque hay firmas pendientes",
        );
      }
      ...
      ServerLogger.error(
        `Error al finalizar acta firmada ${minutesId}:`,
        error,
      );
      throw error;
    }
