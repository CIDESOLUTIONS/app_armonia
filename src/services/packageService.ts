...
export class PackageService {
  ...
  async updatePackage(
    ...
    if (
      data.residentId !== packageData.residentId ||
  ...
  async changePackageStatus(
    ...
    if (
      !this.isValidStatusTransition(
        packageData.status as any,
        data.newStatus as any,
      )
    ) {
      throw new Error(
        `Transición de estado inválida: ${packageData.status} -> ${data.newStatus}`,
      );
    }
   ...
   */
  async getPackageStats(params: { startDate?: string; endDate?: string }) {
    ...
    const [
      ...
      prisma.package.count({
        where: {
          ...dateFilter,
          status: { in: ["RECEIVED", "NOTIFIED", "PENDING"] },
        },
      }),
      ...
      prisma.$queryRaw`
        ...
        WHERE ${
          dateFilter.receivedAt
            ? `"receivedAt" >= '${dateFilter.receivedAt.gte.toISOString()}'::timestamp 
              AND "receivedAt" <= '${dateFilter.receivedAt.lte?.toISOString() || new Date().toISOString()}'::timestamp`
            : "1=1"
        }
        GROUP BY "type"
      `,
    ...
    if (!template) {
      template = await prisma.packageNotificationTemplate.create({
        data: {
          ...
          template:
            "Estimado/a {{residentName}}, ha recibido un paquete de tipo {{packageType}} en recepción. Por favor, pase a recogerlo presentando su identificación. Código de seguimiento: {{trackingCode}}.",
          isDefault: true,
          isActive: true,
        },
      });
  ...
  private generateNotificationContent(template: any, packageData: any): string {
    ...
    content = content.replace(
      /{{receivedAt}}/g,
      packageData.receivedAt.toLocaleString(),
    );
