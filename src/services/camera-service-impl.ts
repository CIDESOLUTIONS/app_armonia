...
export default class CameraService {
  ...
  async createCamera(data: any, userId: number): Promise<any> {
    try {
      ...
      this.checkCameraStatus(camera.id).catch((err) => {
        ServerLogger.warn(
          `Error al verificar estado inicial de cámara ${camera.id}:`,
          err,
        );
      });
  ...
  async updateCamera(id: number, data: any, userId: number): Promise<any> {
    try {
      ...
      if (
        ...
        cameraData.ipAddress !== existingCamera.ipAddress
      ) {
      ...
      if (
        ...
        this.checkCameraStatus(camera.id).catch((err) => {
          ServerLogger.warn(
            `Error al verificar estado de cámara ${camera.id} tras actualización:`,
            err,
          );
        });
      }
  ...
  async toggleCameraActive(
    ...
    try {
      ...
      ServerLogger.error(
        `Error al ${isActive ? "activar" : "desactivar"} cámara ${id}:`,
        error,
      );
      throw error;
    }
      ...
      const updatedCamera = await this.prisma.camera.update({
        ...
        data: {
          ...
          ...(statusDetails && {
            ...
            firmwareVersion:
              statusDetails.firmwareVersion || camera.firmwareVersion,
  ...
  async getCameraStream(
    ...
    try {
      ...
      const streamSettings = camera.streamSettings || {};
  ...
  async stopRecording(id: number, userId: number): Promise<any> {
    try {
      ...
      const duration = Math.round(
        (endTime.getTime() - activeRecording.startTime.getTime()) / 1000,
      );
  ...
  async getCameraRecordings(id: number, filters: any = {}): Promise<any> {
    try {
      ...
      ServerLogger.error(
        `Error al obtener grabaciones de cámara ${id}:`,
        error,
      );
  ...
  async getCameraSnapshots(id: number, filters: any = {}): Promise<any> {
    try {
      ...
      ServerLogger.error(
        `Error al obtener instantáneas de cámara ${id}:`,
        error,
      );
  ...
  async updateCameraPermissions(
    ...
    try {
      ...
      ServerLogger.error(
        `Error al actualizar permisos de cámara ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async checkUserPermission(
    ...
    try {
      ...
      ServerLogger.error(
        `Error al verificar permiso de usuario ${userId} para cámara ${cameraId}:`,
        error,
      );
      return false;
    }
  ...
  private captureSnapshot(
    ...
    return new Promise((resolve, reject) => {
      ...
      let ffmpegArgs = [
        ...
        "-rtsp_transport",
        "tcp", // Usar TCP para RTSP
        "-i",
        rtspUrl, // URL de entrada
        "-frames:v",
        "1", // Capturar un solo frame
        "-q:v",
        "2", // Calidad de imagen (2 es alta)
  ...
  private recordCamera(
    ...
    const ffmpegArgs = [
      ...
      "tcp", // Usar TCP para RTSP
      "-i",
      camera.rtspUrl, // URL de entrada
      "-c:v",
      "libx264", // Codec de video
      "-preset",
      "medium", // Preset de codificación
      "-crf",
      "23", // Factor de calidad constante
      "-c:a",
      "aac", // Codec de audio
      "-strict",
      "experimental",
      "-b:a",
      "128k", // Bitrate de audio
    ];
      ...
      try {
        ...
        if (code === 0) {
          ...
          this.generateThumbnail(outputPath, thumbnailPath)
            ...
            .catch((err) => {
              ServerLogger.warn(
                `Error al generar thumbnail para grabación ${recordingId}:`,
                err,
              );
            });
        } else {
          ...
          ServerLogger.error(
            `Error en grabación ${recordingId}: FFmpeg salió con código ${code}`,
          );
          ServerLogger.error(errorOutput);
        }
      } catch (error) {
        ServerLogger.error(
          `Error al finalizar grabación ${recordingId}:`,
          error,
        );
      }
    });
    ...
    ffmpeg.on("error", async (err) => {
      try {
        ...
        ServerLogger.error(
          `Error al ejecutar FFmpeg para grabación ${recordingId}:`,
          err,
        );
      } catch (error) {
        ServerLogger.error(
          `Error al actualizar estado de grabación ${recordingId}:`,
          error,
        );
      }
    });
  ...
  private generateThumbnail(
    ...
    return new Promise((resolve, reject) => {
      ...
      const ffmpegArgs = [
        "-y", // Sobrescribir archivo si existe
        "-i",
        videoPath, // Video de entrada
        "-ss",
        "00:00:01", // Posición (1 segundo)
        "-vframes",
        "1", // Capturar un solo frame
        "-q:v",
        "2", // Calidad de imagen
