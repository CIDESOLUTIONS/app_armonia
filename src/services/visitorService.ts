...
export const createPreRegisteredVisitor = async (
  visitorData: Omit<PreRegisteredVisitor, "id" | "qrCode">,
): Promise<PreRegisteredVisitor> => {
  const response = await apiClient.post("/visitors/pre-register", visitorData);
  return response.data;
};

export const getPreRegisteredVisitors = async (): Promise<
...
export const registerPackage = async (
  packageData: Omit<Package, "id" | "status" | "createdAt" | "deliveredAt">,
): Promise<Package> => {
...
export const createVisitor = async (
  visitorData: Omit<Visitor, "id" | "entryTime" | "exitTime">,
): Promise<Visitor> => {
