...
export class IncidentService {
  ...
  async createIncident(data: {
    ...
    const sla = await this.getApplicableSLA(
      data.category as any,
      data.priority as any,
    );
    const dueDate = sla
      ? addDays(new Date(), Math.ceil(sla.resolutionTime / (24 * 60)))
      : undefined;
  ...
  async updateIncident(
    ...
    if (data.category !== undefined || data.priority !== undefined) {
      const category =
        data.category !== undefined ? data.category : incident.category;
      const priority =
        data.priority !== undefined ? data.priority : incident.priority;

      const sla = await this.getApplicableSLA(category as any, priority as any);
      if (sla) {
        updateData.slaId = sla.id;
        updateData.dueDate = addDays(
          incident.reportedAt,
          Math.ceil(sla.resolutionTime / (24 * 60)),
        );
      }
    }
  ...
  async changeIncidentStatus(
    ...
    if (
      ...
      throw new Error(
        `Transición de estado inválida: ${incident.status} -> ${data.newStatus}`,
      );
    }
    ...
    if (data.newStatus === "RESOLVED") {
      ...
      if (incident.slaId) {
        updateData.resolutionTime = differenceInMinutes(
          new Date(),
          incident.reportedAt,
        );
        ...
        if (sla) {
          updateData.slaBreached =
            updateData.resolutionTime > sla.resolutionTime;
        }
      }
    }
  ...
  async notifyStatusChange(incidentId: number, newStatus: string) {
    ...
    const template = await this.getNotificationTemplate(
      "EMAIL",
      `incident_${newStatus.toLowerCase()}`,
    );
   ...
   */
  async getIncidentStats(params: { startDate?: string; endDate?: string }) {
    ...
    const [
      ...
      prisma.incident.count({
        where: {
          ...dateFilter,
          status: {
            in: ["REPORTED", "ASSIGNED", "IN_PROGRESS", "ON_HOLD", "REOPENED"],
          },
        },
      }),
      ...
      prisma.$queryRaw`
        ...
        WHERE ${
          dateFilter.reportedAt
            ? `"reportedAt" >= '${dateFilter.reportedAt.gte.toISOString()}'::timestamp 
              AND "reportedAt" <= '${dateFilter.reportedAt.lte?.toISOString() || new Date().toISOString()}'::timestamp`
            : "1=1"
        }
        GROUP BY "category"
      `,
    ...
    if (!template) {
      ...
      switch (eventType) {
        case "incident_created":
          ...
          templateContent =
            "Se ha reportado un nuevo incidente {{incidentNumber}}: {{title}}. Categoría: {{category}}, Prioridad: {{priority}}.";
          break;
        case "incident_assigned":
          subject = "Incidente asignado";
          templateContent =
            "Se le ha asignado el incidente {{incidentNumber}}: {{title}}. Por favor, revise los detalles y comience a trabajar en él.";
          break;
        case "incident_in_progress":
          subject = "Incidente en progreso";
          templateContent =
            "El incidente {{incidentNumber}}: {{title}} está siendo atendido.";
          break;
        case "incident_resolved":
          subject = "Incidente resuelto";
          templateContent =
            "El incidente {{incidentNumber}}: {{title}} ha sido resuelto. Resolución: {{resolution}}.";
          break;
        case "incident_closed":
          subject = "Incidente cerrado";
          templateContent =
            "El incidente {{incidentNumber}}: {{title}} ha sido cerrado.";
          break;
        case "incident_comment":
          subject = "Nuevo comentario en incidente";
          templateContent =
            "{{commentAuthor}} ha comentado en el incidente {{incidentNumber}}: {{title}}. Comentario: {{commentContent}}.";
          break;
      }
