...
class AssemblyService {
  ...
  async getAssembly(id: number): Promise<Assembly> {
    try {
      return await get<Assembly>(`/api/assemblies/${id}`, {
        schema: this.schema,
      });
    } catch (error) {
  ...
  async createAssembly(data: CreateAssemblyDto): Promise<Assembly> {
    try {
      return await post<Assembly>("/api/assemblies", data, {
        schema: this.schema,
      });
    } catch (error) {
  ...
  async updateAssembly(id: number, data: UpdateAssemblyDto): Promise<Assembly> {
    try {
      return await put<Assembly>(`/api/assemblies/${id}`, data, {
        schema: this.schema,
      });
    } catch (error) {
   ...
   */
  async changeAssemblyStatus(
    id: number,
    status: AssemblyStatus,
  ): Promise<Assembly> {
    try {
      return await put<Assembly>(
        `/api/assemblies/${id}/status`,
        { status },
        { schema: this.schema },
      );
    } catch (error) {
  ...
  async getAttendance(id: number): Promise<Attendance[]> {
    try {
      return await get<Attendance[]>(`/api/assemblies/${id}/attendance`, {
        schema: this.schema,
      });
    } catch (error) {
      ServerLogger.error(
        `Error al obtener asistencia para asamblea ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async registerAttendance(data: RegisterAttendanceDto): Promise<Attendance> {
    try {
      return await post<Attendance>("/api/assemblies/attendance", data, {
        schema: this.schema,
      });
    } catch (error) {
  ...
  async createVoting(data: CreateVotingDto): Promise<Voting> {
    try {
      return await post<Voting>("/api/assemblies/votings", data, {
        schema: this.schema,
      });
    } catch (error) {
  ...
  async getVotings(assemblyId: number): Promise<Voting[]> {
    try {
      return await get<Voting[]>(`/api/assemblies/${assemblyId}/votings`, {
        schema: this.schema,
      });
    } catch (error) {
      ServerLogger.error(
        `Error al obtener votaciones para asamblea ${assemblyId}:`,
        error,
      );
      throw error;
    }
  ...
  async startVoting(id: number): Promise<Voting> {
    try {
      return await put<Voting>(
        `/api/assemblies/votings/${id}/start`,
        {},
        { schema: this.schema },
      );
    } catch (error) {
  ...
  async closeVoting(id: number): Promise<Voting> {
    try {
      return await put<Voting>(
        `/api/assemblies/votings/${id}/close`,
        {},
        { schema: this.schema },
      );
    } catch (error) {
  ...
  async castVote(data: CastVoteDto): Promise<Vote> {
    try {
      return await post<Vote>("/api/assemblies/votes", data, {
        schema: this.schema,
      });
    } catch (error) {
