...
const { ASSEMBLY_STATUS } = require('../lib/constants');

const logger = new ServerLogger('AssemblyAdvancedService');

/**
 * Clase que gestiona funcionalidades avanzadas de asambleas
 */
class AssemblyAdvancedService {
  /**
   ...
   * @param {string} assemblyId - ID de la asamblea
   * @returns {Promise<Object>} - Información de quórum
   */
  async calculateQuorum(assemblyId) {
    try {
      if (!assemblyId) {
   ...
   */
  async registerAttendance(assemblyId, userId, unitId, options = {}) {
    try {
   ...
   * @private
   */
  async notifyAttendanceChange(assembly, attendance, action) {
    try {
   ...
   * @param {string} assemblyId - ID de la asamblea
   * @param {Object} quorumData - Datos de quórum
   * @private
   */
  async broadcastQuorumUpdate(assemblyId, quorumData) {
    try {
   ...
   */
  async createVote(assemblyId, voteData) {
    try {
      if (!assemblyId) {
   ...
   * @param {Object} vote - Datos de la votación
   * @param {Object} assembly - Datos de la asamblea
   * @private
   */
  async broadcastVoteCreation(vote, assembly) {
    try {
      websocketService.broadcastToSchema(
        this.schemaName,
        {
   ...
   * @returns {Promise<Object>} - Registro de voto
   */
  async castVote(voteId, userId, unitId, option) {
    try {
   ...
   * @param {Object} vote - Datos de la votación
   * @param {Object} voteRecord - Registro de voto
   * @param {string} action - Acción realizada
   * @private
   */
  async notifyVoteChange(vote, voteRecord, action) {
    try {
      // Obtener datos del usuario y unidad
      const user = await this.prisma.user.findUnique({
   ...
   * @param {string} voteId - ID de la votación
   * @returns {Promise<Object>} - Resultados de la votación
   */
  async calculateVoteResults(voteId) {
    try {
      if (!voteId) {
   ...
   * @param {string} voteId - ID de la votación
   * @param {Object} results - Resultados de la votación
   * @private
   */
  async broadcastVoteResults(voteId, results) {
    try {
      websocketService.broadcastToSchema(
        this.schemaName,
        {
   ...
   * @param {string} voteId - ID de la votación
   * @returns {Promise<Object>} - Votación actualizada
   */
   ...
   * @private
   */
  async broadcastVoteEnd(vote, results) {
    try {
      websocketService.broadcastToSchema(
        this.schemaName,
        {
   ...
   * @param {string} assemblyId - ID de la asamblea
   * @returns {Promise<Object>} - Datos del acta generada
   */
  async generateMinutes(assemblyId) {
    try {
      if (!assemblyId) {
...
module.exports = AssemblyAdvancedService;
