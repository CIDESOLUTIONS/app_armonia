...
export async function getPersonalTransactions(filters?: PersonalTransactionFilterParams): Promise<PersonalTransaction[]> {
  const query = new URLSearchParams();
  if (filters) {
    for (const key in filters) {
      if (filters[key as keyof PersonalTransactionFilterParams]) {
        query.append(key, filters[key as keyof PersonalTransactionFilterParams] as string);
      }
    }
  }
...
export async function createPersonalTransaction(data: CreatePersonalTransactionDto): Promise<PersonalTransaction> {
...
export async function updatePersonalTransaction(id: number, data: UpdatePersonalTransactionDto): Promise<PersonalTransaction> {
