...
export class BillingEngine {
  ...
  async createPayment(request: PaymentRequest): Promise<PaymentResult> {
    try {
      ...
      const expiresAt =
        data.expiresAt ||
        new Date(Date.now() + paymentSettings.paymentExpiry * 60 * 60 * 1000);
      ...
      throw new Error(
        `Error creando pago: ${error instanceof Error ? error.message : "Error desconocido"}`,
      );
    }
  }
  ...
  async createReservationPayment(
    ...
    try {
      ...
      throw new Error(
        `Error creando pago de reserva: ${error instanceof Error ? error.message : "Error desconocido"}`,
      );
    }
  }
  ...
  async confirmPayment(
    ...
    try {
      ...
      if (!validation.success) {
        throw new Error(
          `Datos de confirmación inválidos: ${validation.error.message}`,
        );
      }
      ...
      console.log(
        `[BILLING ENGINE] Pago ${transactionId} confirmado con status: ${data.status}`,
      );
    } catch (error) {
      console.error("[BILLING ENGINE] Error confirmando pago:", error);
      throw new Error(
        `Error confirmando pago: ${error instanceof Error ? error.message : "Error desconocido"}`,
      );
    }
  }
  ...
  async getTransactionStatus(transactionId: string): Promise<{
    ...
    try {
      ...
      console.error(
        "[BILLING ENGINE] Error obteniendo estado de transacción:",
        error,
      );
      throw new Error(
        `Error obteniendo estado de transacción: ${error instanceof Error ? error.message : "Error desconocido"}`,
      );
    }
  }
  ...
  async processRefund(
    ...
    try {
      ...
      throw new Error(
        `Error procesando reembolso: ${error instanceof Error ? error.message : "Error desconocido"}`,
      );
    }
  }
