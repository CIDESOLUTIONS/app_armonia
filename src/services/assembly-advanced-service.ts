...
export class AssemblyAdvancedService {
  ...
  constructor(schemaName: string) {
    if (!schemaName) {
      throw new Error(
        "Se requiere un nombre de esquema para el servicio avanzado de asambleas",
      );
    }
    ...
    logger.info(
      `Servicio avanzado de asambleas inicializado para esquema: ${schemaName}`,
    );
  }
  ...
  async notifyAttendanceChange(
    ...
    try {
      ...
      if (!user || !unit) {
        logger.warn(
          "No se pudo notificar cambio de asistencia: usuario o unidad no encontrados",
        );
        return;
      }
  ...
  async broadcastQuorumUpdate(
    ...
    try {
      ...
      logger.info(
        `Actualización de quórum transmitida para asamblea ${assemblyId}`,
      );
    } catch (error: any) {
      logger.error(
        `Error al transmitir actualización de quórum: ${error.message}`,
      );
    }
  }
  ...
  async broadcastVoteCreation(vote: any, assembly: any): Promise<void> {
    try {
      ...
      logger.info(
        `Notificación de nueva votación transmitida para asamblea ${assembly.id}`,
      );
    } catch (error: any) {
      logger.error(
        `Error al transmitir creación de votación: ${error.message}`,
      );
    }
  }
  ...
  async notifyVoteChange(
    ...
    try {
      ...
      if (!user || !unit) {
        logger.warn(
          "No se pudo notificar cambio de voto: usuario o unidad no encontrados",
        );
        return;
      }
  ...
  async broadcastVoteResults(vote: any, results: any): Promise<void> {
    try {
      ...
      logger.info(
        `Resultados de votación transmitidos para votación ${voteId}`,
      );
    } catch (error: any) {
      logger.error(
        `Error al transmitir resultados de votación: ${error.message}`,
      );
    }
  }
  ...
  async broadcastVoteEnd(vote: any, results: any): Promise<void> {
    try {
      ...
      logger.info(
        `Finalización de votación transmitida para votación ${vote.id}`,
      );
    } catch (error: any) {
      logger.error(
        `Error al transmitir finalización de votación: ${error.message}`,
      );
    }
  }
