...
const MOCK_PQRS: PQR[] = [
  {
    ...
    description:
      "Hay una gotera grande en el parqueadero comunal que está afectando mi vehículo.",
    ...
    response:
      "Se realizó la reparación del techo y se selló la filtración. Quedamos pendientes de cualquier novedad adicional.",
  ...
  },
  {
    ...
    description:
      "Solicito revisión de la cuota extraordinaria asignada a mi inmueble, ya que considero que hay un error en el cálculo según el coeficiente.",
  ...
  },
  {
    ...
    description:
      "Durante el mantenimiento de fachada, los operarios dañaron una ventana de mi apartamento. Solicito reparación urgente.",
    type: PQRType.CLAIM,
  ...
  },
  {
    ...
    description:
      "El apartamento B-402 realiza fiestas con música a alto volumen después de las 10 pm regularmente.",
    type: PQRType.COMPLAINT,
    status: PQRStatus.PENDING,
    priority: PQRPriority.MEDIUM,
  ...
  },
  {
    ...
    description:
      "Solicito autorización para realizar una renovación en mi cocina que incluirá cambio de pisos y muebles.",
    type: PQRType.PETITION,
    status: PQRStatus.RESOLVED,
    ...
    assignedToName: "Carlos Martínez",
    response:
      "Solicitud aprobada. Por favor coordinar con administración el horario de trabajo y el ingreso de materiales y personal.",
    category: "infrastructure",
...
export const usePQRService = () => {
   ...
   */
  const getPQRs = useCallback(
    async (filters: PQRFilters = {}): Promise<PQRResponse> => {
      ...
      if (search) {
        ...
        filtered = filtered.filter(
          (pqr) =>
            ...
            (pqr.residentName &&
              pqr.residentName.toLowerCase().includes(searchLower)),
        );
      }
   ...
   */
  const updatePQR = useCallback(
    async (id: number, data: Partial<PQR>): Promise<PQR | null> => {
      // Simular una llamada a la API
      await new Promise((resolve) => setTimeout(resolve, 800));
