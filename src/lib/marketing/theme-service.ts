...
class ThemeService {
  ...
  public async getUserTheme(userId: number): Promise<Theme> {
    try {
      ...
      logger.error(
        `Error getting user theme: ${error instanceof Error ? error.message : "Error desconocido"}`,
        { userId },
      );
    }
  ...
  public async setUserTheme(userId: number, themeId: string): Promise<boolean> {
    try {
      ...
      logger.error(
        `Error setting user theme: ${error instanceof Error ? error.message : "Error desconocido"}`,
        {
          userId,
          themeId,
        },
