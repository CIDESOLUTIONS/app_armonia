/**
 * @fileoverview Constantes de metadatos para la aplicación Armonía.
 * Define tipos de entidades, campos de metadatos y validadores.
 */
...
/**
 * Define la estructura de un campo de metadatos.
 */
...
/**
 * Campos de metadatos por cada tipo de entidad.
 */
export const METADATA_FIELDS: Record<EntityType, MetadataField[]> = {
  [ENTITY_TYPES.RESIDENT]: [
  ...
  [ENTITY_TYPES.COMPLEX]: [
    ...
    { name: 'administrator_phone', type: 'phone', required: true },
    { name: 'administrator_email', type: 'email', required: true },
  ],
...
/**
 * Funciones de validación para tipos de metadatos.
 */
export const VALIDATORS: Record<string, (value: any) => boolean> = {
  ...
  date: (value): value is Date => value instanceof Date && !isNaN(value.getTime()),
  email: (value): value is string =>
    typeof value === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value),
  phone: (value): value is string =>
    typeof value === 'string' && /^[0-9+\-\s()]{7,15}$/.test(value),
};
