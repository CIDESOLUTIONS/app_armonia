...
export async function runCommunicationsTests() {
  ...
  try {
    ...
    const readNotification = await markNotificationAsRead(
      notification.id,
      testUserId,
    );
    ...
    console.log(
      "Notificación con confirmación enviada:",
      confirmationNotification.id,
    );

    // Prueba 5: Confirmar lectura de notificación
    console.log("Prueba 5: Confirmando lectura de notificación...");
    const confirmedNotification = await confirmNotificationReading(
      confirmationNotification.id,
      testUserId,
    );
