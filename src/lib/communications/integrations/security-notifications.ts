 ...
 */
export async function notifyFailedLoginAttempt(
  userId: number,
  ipAddress: string,
  userAgent: string,
  attemptCount: number,
  ...
  try {
    ...
    console.error(
      "Error al enviar notificación de intento de acceso fallido:",
      error,
    );
    throw error;
  }
 ...
 */
export async function notifyUnusualAccess(
  userId: number,
  ipAddress: string,
  location: string,
  deviceInfo: string,
...
export async function notifyPasswordChanged(userId: number) {
  try {
    ...
    await notifyUser(userId, {
      ...
      message:
        "Su contraseña ha sido actualizada correctamente. Si no realizó este cambio, contacte al administrador inmediatamente.",
      priority: "high",
      requireConfirmation: true,
      data: {
        timestamp: new Date().toISOString(),
  ...
  try {
    ...
    if (affectedUserId) {
      await notifyUser(affectedUserId, {
        ...
        message:
          "Se ha detectado actividad inusual en su cuenta. Por favor verifique sus accesos recientes.",
        priority,
        link: "/resident/profile/security",
        data: {
          severity,
    ...
    console.error(
      "Error al enviar notificación de actividad sospechosa:",
      error,
    );
    throw error;
  }
...
export async function notifySessionTimeout(userId: number) {
  try {
    ...
    await notifyUser(userId, {
      ...
      message:
        "Su sesión ha sido cerrada automáticamente por inactividad. Por favor inicie sesión nuevamente.",
      priority: "low",
      data: {
        timestamp: new Date().toISOString(),
      },
 ...
 */
export async function notifySecurityUpdate(
  title: string,
  details: string,
  ...
  try {
    ...
    console.error(
      "Error al enviar notificación de actualización de seguridad:",
      error,
    );
    throw error;
  }
...
export async function processAuditEvent(
  ...
  try {
    ...
    switch (action) {
      ...
      case AuditActionType.ACCESS_ATTEMPT:
        if (userId && status === AuditStatus.FAILURE) {
          await notifySuspiciousActivity(details, "medium", userId);
        }
        break;
    ...
    console.error(
      "Error al procesar evento de auditoría para notificaciones:",
      error,
    );
