...
export const WebSocketService = {
  sendToClient: jest.fn((schemaName: string, userId: number, message: any) => {
    logger.info(
      `[MOCK] WebSocketService.sendToClient - Schema: ${schemaName}, User: ${userId}, Message: ${JSON.stringify(message)}`,
    );
  ...
  broadcastToSchema: jest.fn((schemaName: string, message: any) => {
    logger.info(
      `[MOCK] WebSocketService.broadcastToSchema - Schema: ${schemaName}, Message: ${JSON.stringify(message)}`,
    );
    return Promise.resolve(true);
  }),
