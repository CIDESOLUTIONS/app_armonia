...
export type RealTimeEvent =
  ...
  | {
      type: "status_change";
      data: { userId: number; status: "online" | "offline" | "away" };
    }
  | {
      type: "typing";
      data: { userId: number; conversationId: string; isTyping: boolean };
    }
  | {
      type: "assembly_update";
      data: { assemblyId: string; status: string; quorum: number };
    }
  | {
      type: "visitor_arrival";
      data: { visitorId: string; visitorName: string; unit: string };
    };
...
// Crear el contexto
const RealTimeCommunicationContext = createContext<
  RealTimeCommunicationContextType | undefined
>(undefined);

// Proveedor del contexto
export function RealTimeCommunicationProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const unreadMessagesCount = Object.values(messages)
    .flat()
      ...
      if (response.ok) {
        setMessages((prev) => {
          ...
          Object.keys(newMessages).forEach((senderId) => {
            newMessages[Number(senderId)] = newMessages[Number(senderId)].map(
...
export function useRealTimeCommunication() {
  ...
  if (context === undefined) {
    throw new Error(
      "useRealTimeCommunication debe ser usado dentro de un RealTimeCommunicationProvider",
    );
  }
