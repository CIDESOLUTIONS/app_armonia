...
export async function sendPushNotification(
  ...
  try {
    if (!options || !options.deviceToken) {
      throw new Error(
        "Se requiere un token de dispositivo para enviar notificación push",
      );
    }
    ...
    logger.info(
      `[MOCK] Enviando notificación push a dispositivo ${options.deviceToken}: ${options.title}`,
    );
...
export async function sendMulticastPushNotification(
  ...
  try {
    if (
      ...
      throw new Error(
        "Se requiere al menos un token de dispositivo para envío multicast",
      );
    }
    ...
    logger.info(
      `[MOCK] Enviando notificación push multicast a ${deviceTokens.length} dispositivos: ${notification.title}`,
    );
 ...
 * @returns Resultado del envío
 */
export async function sendPQRPushNotification(
  pqr: any,
  ...
  try {
    ...
    logger.error(
      `Error al enviar notificación push de PQR: ${error instanceof Error ? error.message : "Error desconocido"}`,
    );
    return {
      success: false,
      error: error instanceof Error ? error.message : "Error desconocido",
 ...
 * @returns Resultado del envío
 */
export async function sendAssemblyPushNotification(
  assembly: any,
  user: { deviceToken: string },
  action: string,
): Promise<any> {
  try {
    ...
    const formattedDate = new Date(assembly.date).toLocaleDateString(
      "es-ES",
      dateOptions,
    );
...
export async function sendPaymentPushNotification(
  ...
  try {
    ...
    logger.error(
      `Error al enviar notificación push de pago: ${error instanceof Error ? error.message : "Error desconocido"}`,
    );
    return {
      success: false,
      error: error instanceof Error ? error.message : "Error desconocido",
...
export async function registerDeviceToken(
  ...
  try {
    if (!userId || !deviceToken) {
      throw new Error(
        "Se requieren ID de usuario y token de dispositivo para registro",
      );
    }
    ...
    logger.info(
      `[MOCK] Registrado token de dispositivo para usuario ${userId}: ${deviceToken}`,
    );
    ...
    logger.error(
      `Error al registrar token de dispositivo: ${error instanceof Error ? error.message : "Error desconocido"}`,
    );
    return {
      success: false,
      error: error instanceof Error ? error.message : "Error desconocido",
...
export async function unregisterDeviceToken(
  ...
  try {
    if (!userId || !deviceToken) {
      throw new Error(
        "Se requieren ID de usuario y token de dispositivo para eliminación",
      );
    }
    ...
    logger.info(
      `[MOCK] Eliminado token de dispositivo para usuario ${userId}: ${deviceToken}`,
    );
    ...
    logger.error(
      `Error al eliminar token de dispositivo: ${error instanceof Error ? error.message : "Error desconocido"}`,
    );
    return {
      success: false,
      error: error instanceof Error ? error.message : "Error desconocido",
