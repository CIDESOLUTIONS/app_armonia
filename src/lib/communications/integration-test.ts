...
import {
  runCommunicationsTests,
  validateCommunicationsIntegration,
} from "./validation";
...
async function runIntegrationTests() {
  ...
  try {
    ...
    if (testsResult.success) {
      ...
      console.error(
        "❌ Error en las pruebas de componentes:",
        testsResult.error,
      );
    }
    ...
    if (integrationResult.success) {
      ...
      console.log(
        "Resultados:",
        JSON.stringify(integrationResult.results, null, 2),
      );
    } else {
      console.error(
        "❌ Error en la validación de integración:",
    ...
    if (allSuccess) {
      ...
      console.log(
        "El sistema de comunicaciones está funcionando correctamente",
      );
    } else {
      console.error("❌ SE ENCONTRARON ERRORES EN LAS PRUEBAS");
      console.error(
        "Revise los detalles anteriores para identificar los problemas",
      );
    }
