...
export async function generatePasswordResetToken(
  ...
  try {
    ...
    ServerLogger.error(
      "Error generando token de restablecimiento de contraseña:",
      error,
    );
...
export async function verifyPasswordResetToken(
  ...
  try {
    ...
    if (payload.type !== "password-reset") {
      ServerLogger.warn(
        "Token de restablecimiento de contraseña inválido: tipo incorrecto",
      );
      return null;
    }
    ...
    if (!payload.id || !payload.email) {
      ServerLogger.warn(
        "Token de restablecimiento de contraseña con datos incompletos",
      );
      return null;
    }
...
export async function verifyAuth(request: NextRequest) {
  try {
    ...
    ServerLogger.debug("Token verificado correctamente", {
      userId: payload.id,
    });
