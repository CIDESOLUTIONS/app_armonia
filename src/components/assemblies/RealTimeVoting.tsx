  ...
  const fetchVotingStats = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/assemblies/${assemblyId}/agenda/${agendaNumeral}/votes`,
      );
      ...
      if (!response.ok) {
        throw new Error(
          language === "Español"
            ...
            : "Error loading voting statistics",
        );
      }
  ...
  return (
    <Card className="p-4 mb-2">
      <div className="flex justify-between items-start mb-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            {language === "Español" ? "Votos totales:" : "Total votes:"}{" "}
            {stats.totalVotes}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div
          className={`p-3 rounded-md flex items-center justify-between ${userVote === "YES" ? "bg-green-50 border border-green-200" : "bg-gray-50"}`}
        >
          <div className="flex items-center">
            <CheckCircle2
              className={`w-4 h-4 mr-2 ${userVote === "YES" ? "text-green-600" : "text-gray-400"}`}
            />
            <span className="text-sm font-medium">
              {language === "Español" ? "A favor" : "In favor"}
            </span>
          </div>
          <div>
        ...
        <div
          className={`p-3 rounded-md flex items-center justify-between ${userVote === "NO" ? "bg-red-50 border border-red-200" : "bg-gray-50"}`}
        >
          <div className="flex items-center">
            <XCircle
              className={`w-4 h-4 mr-2 ${userVote === "NO" ? "text-red-600" : "text-gray-400"}`}
            />
            <span className="text-sm font-medium">
              {language === "Español" ? "En contra" : "Against"}
            </span>
          </div>
          <div>
