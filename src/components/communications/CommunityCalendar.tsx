        ...
        setEvents((prev) =>
          prev.map((event) => {
            if (event.id === eventId) {
              ...
              if (
                existingAttendeeIndex !== undefined &&
                existingAttendeeIndex >= 0
              ) {
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({ length: new Date(daysInMonth[0]).getDay() }).map(
                (_, index) => (
                  <div
                    key={`empty-${index}`}
                    className="h-24 p-1 border rounded-md bg-gray-50"
                  ></div>
                ),
              )}
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span
                        className={`text-sm font-medium ${isToday(day) ? "bg-blue-500 text-white rounded-full w-5 h-5 flex items-center justify-center" : ""}`}
                      >
                        {format(day, "d")}
                      </span>
                    ...
                    <div className="space-y-1">
                      {dayEvents.slice(0, 2).map((event) => (
                        ...
                        >
                          <div
                            className={`h-2 w-2 rounded-full mr-1 ${getEventColor(event.type)}`}
                          ></div>
                          {event.title}
                        </div>
                      ))}
                  ...
                  .map((event) => (
                    ...
                    >
                      <div className="flex items-start">
                        <div
                          className={`h-2 w-2 mt-1.5 rounded-full mr-2 ${getEventColor(event.type)}`}
                        />
                        <div className="flex-1">
                          ...
                          {event.maxAttendees && (
                            <div className="flex items-center gap-2 mt-1 text-sm text-gray-500">
                              ...
                              <span>
                                {countAttendees(event, "attending")}/
                                {event.maxAttendees}{" "}
                                {language === "Español"
                                  ? "asistentes"
                                  : "attendees"}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <Badge className="bg-green-500">
                      {countAttendees(selectedEvent, "attending")}{" "}
                      {language === "Español" ? "asistirán" : "attending"}
                    </Badge>
                    <Badge
                      variant="outline"
                      className="text-yellow-500 border-yellow-500"
                    >
                      {countAttendees(selectedEvent, "maybe")}{" "}
                      {language === "Español" ? "tal vez" : "maybe"}
                    </Badge>
                    <Badge
                      variant="outline"
                      className="text-red-500 border-red-500"
                    >
                      {countAttendees(selectedEvent, "not_attending")}{" "}
                      {language === "Español"
                        ? "no asistirán"
                        : "not attending"}
                    </Badge>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <Button
                    variant={
                      getUserAttendanceStatus(selectedEvent) === "attending"
                        ? "default"
                        : "outline"
                    }
                    size="sm"
                    onClick={() =>
                      confirmAttendance(selectedEvent.id, "attending")
                    }
                    disabled={
                  ...
                  <Button
                    variant={
                      getUserAttendanceStatus(selectedEvent) === "maybe"
                        ? "default"
                        : "outline"
                    }
                    size="sm"
                  ...
                  <Button
                    variant={
                      getUserAttendanceStatus(selectedEvent) === "not_attending"
                        ? "default"
                        : "outline"
                    }
                    size="sm"
                    onClick={() =>
                      confirmAttendance(selectedEvent.id, "not_attending")
                    }
                  >
