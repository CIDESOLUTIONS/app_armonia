  ...
  return (
    <div className="w-full mb-4">
      ...
      <div className="flex justify-between text-sm mt-1">
        <span
          className={`font-bold ${status === "REACHED" ? "text-green-600" : "text-red-600"}`}
        >
...
const AssemblyLiveView = () => {
  ...
  const loadAssemblyData = useCallback(async () => {
    setLoading(true);
    try {
      const assemblyData = await assemblyService.getAssembly(
        parseInt(id as string),
      );
      setAssembly(assemblyData);

      const attendeesData = await assemblyService.getAttendance(
        parseInt(id as string),
      );
      setAttendees(attendeesData);

      const votingsData = await assemblyService.getVotings(
        parseInt(id as string),
      );
    ...
    (event: any) => {
      ...
      switch (type) {
        ...
        case "VOTING_STARTED": {
          ...
          const startedVoting = votings.find(
            (v: any) => v.id === data.votingId,
          );
          if (startedVoting) {
            setActiveVoting({
              ...startedVoting,
      ...
      <div className="bg-white shadow-md rounded-lg p-3 mb-3">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
          <div className="md:col-span-8">
            <h4 className="text-2xl font-bold mb-1">{assembly.title}</h4>

            <div className="flex items-center mb-1">
              <AssemblyStatusBadge status={assembly.status} />
              <p className="ml-2 text-sm">
                {format(new Date(assembly.date), "PPP", { locale: es })} •{" "}
                {format(new Date(assembly.date), "HH:mm")}
              </p>
              {assembly.endTime && (
                <p className="ml-2 text-sm">
          ...
          <div className="md:col-span-4 flex justify-end gap-2">
            {assembly.status === "SCHEDULED" &&
              assembly.quorumStatus === "REACHED" && (
                <Button variant="default" onClick={handleStartAssembly}>
            ...
            {assembly.status === "IN_PROGRESS" && (
              <Button variant="destructive" onClick={handleEndAssembly}>
          ...
          <Card className="mb-3">
            ...
            <CardContent>
              ...
              <div className="flex justify-between mt-2 text-sm">
                ...
                <p>
                  <strong>Coeficiente total:</strong>{" "}
                  {assembly.currentCoefficient}%
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="mb-3 border-primary">
              ...
              <CardContent>
                ...
                <div className="mt-2 mb-2 text-sm">
                  <p>
                    <strong>Tipo:</strong>{" "}
                    {activeVoting.type === "SIMPLE_MAJORITY"
                      ? "Mayoría simple"
                      : activeVoting.type === "QUALIFIED_MAJORITY"
                        ? "Mayoría calificada"
                        : activeVoting.type === "UNANIMOUS"
                          ? "Unanimidad"
                    ...
                    {votings.length === 0 ? (
                      ...
                      votings.map((voting) => (
                        <TableRow key={voting.id}>
                          <TableCell>
                            <p className="font-medium">{voting.title}</p>
                          ...
                          <TableCell>
                            {voting.status === "CLOSED" ? (
                              <Badge
                                variant={
                                  voting.isApproved ? "default" : "destructive"
                                }
                              >
        ...
        <Dialog open={createVotingOpen} onOpenChange={setCreateVotingOpen}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Configure los detalles de la nueva votación.
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button type="button" onClick={() => setCreateVotingOpen(false)}>
                Cancelar
              </Button>
              <Button
                type="submit"
      ...
      {viewResultsOpen && selectedVoting && (
        <Dialog open={viewResultsOpen} onOpenChange={setViewResultsOpen}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>
                Resultados de Votación: {selectedVoting.title}
              </DialogTitle>
              <DialogDescription>
                Detalles y resultados de la votación.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
              <p>
                <strong>Total Votos:</strong> {selectedVoting.totalVotes}
              </p>
