...
export default function StaffDataTable({ data }) {
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => {
      toast({ title: 'Error al eliminar el usuario de personal', variant: 'destructive' });
    },
  });
  ...
  const handleDelete = (id) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este usuario de personal?')) {
