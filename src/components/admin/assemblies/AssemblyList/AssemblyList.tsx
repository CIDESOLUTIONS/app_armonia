    ...
    },
    {
      ...
      render: (value: number, record: Assembly) => (
        <div className="flex items-center space-x-2">
          ...
          <span>
            {record.participantes}/{value} (
            {Math.round((record.participantes / value) * 100)}%)
          </span>
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">Asambleas</h2>
        <Button variant="primary" onClick={onCreateNew}>
          Nueva Asamblea
        </Button>
      </div>

      <Table columns={columns} data={assemblies} onRowClick={onViewDetails} />
    </div>
  );
};
