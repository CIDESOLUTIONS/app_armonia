...
import { Loader2, CheckCircle, XCircle, UserPlus, BarChart2, FileText } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { useToast } from "@/components/ui/use-toast";
import { getAssemblyById, registerAttendance, getAssemblyQuorumStatus, createVote, submitVote, getVoteResults, generateMeetingMinutes, Assembly, CreateVoteDto, SubmitVoteDto, Vote, VoteResult } from "@/services/assemblyService";
...
export default function AssemblyDetailPage() {
  ...
  const handleCreateVote = async () => {
    ...
    try {
      ...
      setNewVoteForm({ assemblyId, question: "", options: ["", ""], isWeighted: false });
    } catch (error) {
    ...
    <div className="container mx-auto p-6">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          ...
          <Button onClick={() => handleRegisterAttendance(user?.id || 0, true)} className="mt-4">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <Button onClick={() => setIsCreateVoteDialogOpen(true)} className="mb-4">
              ...
              <div key={vote.id} className="border p-4 rounded-md mb-4">
                ...
                <div className="mt-2">
                  {vote.options.map((option) => (
                    <Button key={option.id} variant="outline" className="mr-2 mb-2" onClick={() => submitVote({ voteId: vote.id, optionId: option.id, userId: user?.id || 0, weight: 1 })}> {/* Simplified weight for now */}
                      {option.value}
                    </Button>
                  ))}
                </div>
                <Button onClick={() => handleGetVoteResults(vote.id)} className="mt-2">
                ...
                {voteResults[vote.id] && (
                  <div className="mt-4 p-2 border-t">
                    ...
                    {voteResults[vote.id].results.map((result, index) => (
                      <p key={index}>{result.value}: {result.totalWeight} votos</p>
                    ))}
                  </div>
                )}
              </div>
      ...
      <Dialog open={isCreateVoteDialogOpen} onOpenChange={setIsCreateVoteDialogOpen}>
        <DialogContent>
          <DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div>
              ...
              <Input id="voteQuestion" value={newVoteForm.question} onChange={(e) => setNewVoteForm({ ...newVoteForm, question: e.target.value })} />
            </div>
            <div>
                ...
                <div key={index} className="flex gap-2 mb-2">
                  ...
                  {newVoteForm.options.length > 2 && (
                    <Button variant="outline" size="sm" onClick={() => {
                      const newOptions = newVoteForm.options.filter((_, i) => i !== index);
                      setNewVoteForm({ ...newVoteForm, options: newOptions });
                    }}>
              ...
              <Button variant="outline" onClick={() => setNewVoteForm({ ...newVoteForm, options: [...newVoteForm.options, ""] })}>
                Añadir Opción
              </Button>
            </div>
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => setNewVoteForm({ ...newVoteForm, isWeighted: checked as boolean })}
              />
              <Label htmlFor="isWeighted">Votación Ponderada</Label>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsCreateVoteDialogOpen(false)}>Cancelar</Button>
