...
export default function ComplexSetupPage() {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Configuración del Conjunto Residencial</h1>
      ...
      {complexInfo && (
        <form onSubmit={handleSubmit} className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="grid gap-2">
            ...
            <Input id="schemaName" name="schemaName" value={formData.schemaName || ''} onChange={handleInputChange} disabled />
          </div>
          <div className="grid gap-2">
          ...
          {/* Property Types - Consider a multi-select or tags input for this */}
          <div className="grid gap-2 col-span-full">
            <Label htmlFor="propertyTypes">Tipos de Propiedad (separados por coma)</Label>
            <Input 
              id="propertyTypes" 
              name="propertyTypes" 
              value={formData.propertyTypes?.join(', ') || ''} 
              onChange={(e) => setFormData(prev => ({ ...prev, propertyTypes: e.target.value.split(',').map(s => s.trim()) }))}
            />
          </div>
