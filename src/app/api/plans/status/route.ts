...
export async function GET(request: NextRequest) {
  try {
    ...
    const monthlyCost = FreemiumService.calculateMonthlyCost(
      complex.planType,
      complex.totalUnits,
    );

    // Obtener funcionalidades faltantes comparado con planes superiores
    const missingFeaturesStandard =
      complex.planType === "BASIC"
        ? FreemiumService.getMissingFeatures(complex.planType, "STANDARD")
        : [];

    const missingFeaturesPremium =
      complex.planType !== "PREMIUM"
    ...
    const response = {
      ...
      validation: {
        ...
        unitsValidation,
        canAddUnits:
          !isExpired && (isTrialActive || complex.planType !== "BASIC"),
      },
      upgrade: {
        missingFeaturesStandard,
        missingFeaturesPremium,
        recommendedPlan: unitsValidation.isValid
          ? null
          : complex.totalUnits <= 80
            ? "PREMIUM"
            : "ENTERPRISE",
      },
      limits: {
