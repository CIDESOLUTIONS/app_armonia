...
export async function GET(request: NextRequest) {
  try {
    ...
    ServerLogger.info(
      `Residentes listados para el complejo ${payload.complexId}`,
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const tenantPrisma = getTenantPrismaClient(payload.schemaName);
    const newResident = await tenantPrisma.resident.create({
      data: validatedData,
    });

    ServerLogger.info(
      `Residente creado: ${newResident.name} en complejo ${payload.complexId}`,
    );
