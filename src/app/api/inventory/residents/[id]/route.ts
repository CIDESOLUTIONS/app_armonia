...
const ResidentUpdateSchema = z.object({
  ...
  role: z
    .enum(["RESIDENT", "OWNER", "TENANT"], { message: "Rol inválido." })
    .optional(),
...
export async function PUT(
  ...
  try {
    ...
    ServerLogger.info(
      `Residente actualizado: ${updatedResident.name} en complejo ${payload.complexId}`,
    );
...
export async function DELETE(
  ...
  try {
    ...
    ServerLogger.info(
      `Residente eliminado: ID ${id} en complejo ${payload.complexId}`,
    );
    return NextResponse.json(
