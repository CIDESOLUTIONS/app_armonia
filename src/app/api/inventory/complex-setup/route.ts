...
const ComplexInfoSchema = z.object({
  ...
  totalUnits: z
    .number()
    .int()
    .min(0, "El total de unidades debe ser un número positivo."),
  ...
  try {
    ...
    if (!complexInfo) {
      return NextResponse.json(
        { message: "Información del complejo no encontrada" },
        { status: 404 },
      );
    }

    ServerLogger.info(
      `Información del complejo ${payload.complexId} obtenida.`,
    );
    return NextResponse.json(complexInfo, { status: 200 });
  } catch (error) {
    ServerLogger.error("Error al obtener información del complejo:", error);
    return NextResponse.json(
      { message: "Error al obtener información del complejo" },
      { status: 500 },
    );
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!payload.complexId) {
      return NextResponse.json(
        { message: "ID de complejo no encontrado en la sesión" },
        { status: 400 },
      );
    }
    ...
    ServerLogger.info(
      `Información del complejo ${payload.complexId} actualizada.`,
    );
    ...
    return NextResponse.json(
      { message: "Error al actualizar información del complejo" },
      { status: 500 },
    );
  }
}
