...
export async function PUT(
  ...
  try {
    ...
    if (
      session.user.role !== "ADMIN" &&
      currentReservation.userId !== session.user.id
    ) {
      return NextResponse.json(
    ...
    if (error instanceof Error) {
      if (
        error.message === "El horario solicitado no está disponible" ||
...
export async function DELETE(
  ...
  try {
    ...
    const cancellationReason =
      data.cancellationReason || "Cancelada por el usuario";
    ...
    if (error instanceof Error) {
      if (
        error.message.includes("cancelaciones") ||
        error.message === "Reserva no encontrada" ||
