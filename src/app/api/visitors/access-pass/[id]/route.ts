...
export async function POST(
  ...
  if (isValidateRequest) {
    try {
      ...
      // Validar el pase
      const validationResult = await accessPassService.validateAccessPass(
        accessPass.passCode,
      );
  ...
  if (isUsageRequest) {
    try {
      ...
      const result = await accessPassService.registerPassUsage(
        id,
        sanitizedData,
      );
      ...
      return NextResponse.json(
        {
          error: "Error al registrar uso de pase de acceso",
  ...
  if (isRevokeRequest) {
    try {
      ...
      // Revocar pase
      const accessPass = await accessPassService.revokeAccessPass(
        id,
        sanitizedData,
      );
