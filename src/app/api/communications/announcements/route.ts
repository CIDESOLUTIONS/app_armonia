...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!["ADMIN", "COMPLEX_ADMIN"].includes(payload.role)) {
      ...
      where.OR = [{ expiresAt: { gte: new Date() } }, { expiresAt: null }];
    }
    ...
    ServerLogger.info(
      `Anuncios listados para el complejo ${payload.complexId}`,
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    ServerLogger.info(
      `Anuncio creado: ${newAnnouncement.title} por ${payload.email} en complejo ${payload.complexId}`,
    );
