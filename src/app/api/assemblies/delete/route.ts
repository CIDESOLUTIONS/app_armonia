  ...
  try {
    ...
    if (decoded.role !== "COMPLEX_ADMIN") {
      return NextResponse.json(
        {
          message:
            "Acceso denegado: solo administradores pueden eliminar asambleas",
        },
        { status: 403 },
      );
    }
    ...
    if (!id) {
      return NextResponse.json(
        { message: "ID de la asamblea requerido" },
        { status: 400 },
      );
    }
    ...
    if (now > assemblyEnd) {
      return NextResponse.json(
        { message: "No se puede eliminar una asamblea ya finalizada" },
        { status: 403 },
      );
    }
    ...
    return NextResponse.json(
      { message: "Error al eliminar la asamblea", error: error.message },
      { status: 500 },
    );
  } finally {
...
export const DELETE_handler = csrfProtection(
  xssProtection(
    auditMiddleware(
      ...
      (req) =>
        `Eliminación de asamblea: ${new URL(req.url).searchParams.get("id") || "ID no disponible"}`,
    )(DELETE),
  ),
);
