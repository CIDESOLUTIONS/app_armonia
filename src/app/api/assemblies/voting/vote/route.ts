  ...
  try {
    ...
    if (!assemblyId || !agendaNumeral || !["YES", "NO"].includes(vote)) {
      return NextResponse.json(
        { message: "Faltan campos requeridos o voto inválido (YES/NO)" },
        { status: 400 },
      );
    }
    ...
    if (!resident) {
      return NextResponse.json(
        { message: "Usuario no registrado como residente" },
        { status: 403 },
      );
    }
    ...
    if (!assembly || assembly.complexId !== decoded.complexId) {
      return NextResponse.json(
        { message: "Asamblea no encontrada o no autorizada" },
        { status: 404 },
      );
    }
    ...
    if (now < new Date(assembly.date) || now > assemblyEnd) {
      return NextResponse.json(
        { message: "Solo se puede votar durante la asamblea" },
        { status: 403 },
      );
    }
    ...
    if (!attendance || !attendance.verified) {
      return NextResponse.json(
        { message: "No estás verificado para votar en esta asamblea" },
        { status: 403 },
      );
    }

    const votes = assembly.votes || {};
    const agendaVotes = votes[agendaNumeral] || {
      YES: 0,
      NO: 0,
      residents: [],
    };
    if (agendaVotes.residents.includes(resident.id)) {
      return NextResponse.json(
        { message: "Ya has votado en este punto" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { message: "Error al registrar el voto", error: error.message },
      { status: 500 },
    );
  } finally {
