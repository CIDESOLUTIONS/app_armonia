...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !session ||
      !session.user ||
      !(session.user as JWTPayload).isGlobalAdmin
    ) {
    ...
    if (!newRole || !targetUserId) {
      return NextResponse.json(
        { message: "Rol o ID de usuario no proporcionado" },
        { status: 400 },
      );
    }
    ...
    ServerLogger.info(
      `Rol del usuario ${targetUserId} cambiado a ${newRole} por ${session.user.email}`,
    );
