...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!decodedToken) {
      ServerLogger.warn(
        "Intento de restablecimiento de contraseña con token inválido o expirado.",
      );
    ...
    if (!user) {
      ServerLogger.warn(
        `Usuario no encontrado para restablecimiento de contraseña: ${email} en esquema ${schemaName || "principal"}`,
      );
      return NextResponse.json(
    ...
    ServerLogger.info(
      `Contraseña restablecida exitosamente para el usuario: ${email}`,
    );
    return NextResponse.json(
