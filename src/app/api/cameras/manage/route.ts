...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[CAMERA REGISTER] Cámara ${data.name} registrada por ${payload.email}`,
    );
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : "Error registrando cámara",
      },
      { status: 500 },
    );
  }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log(
      `[CAMERA UPDATE] Cámara ${id} actualizada por ${payload.email}`,
    );
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : "Error actualizando cámara",
      },
      { status: 500 },
    );
  }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(
      `[CAMERA DELETE] Cámara ${cameraId} eliminada por ${payload.email}`,
    );
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : "Error eliminando cámara",
      },
      { status: 500 },
    );
  }
