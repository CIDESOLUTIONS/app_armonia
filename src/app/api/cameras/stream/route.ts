...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !["ADMIN", "COMPLEX_ADMIN", "RECEPTION", "RESIDENT"].includes(
        payload.role,
      )
    ) {
    ...
    console.log(
      `[STREAM REQUEST] Stream solicitado para cámara ${cameraId} por ${payload.email}`,
    );
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : "Error obteniendo stream",
      },
      { status: 500 },
    );
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `[SNAPSHOT] Imagen capturada de cámara ${cameraId} por ${payload.email}`,
    );
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : "Error capturando imagen",
      },
      { status: 500 },
    );
  }
