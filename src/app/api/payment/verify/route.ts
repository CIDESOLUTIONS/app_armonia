  ...
  try {
    ...
    try {
      ...
      if (
        ...
        new Date(paymentTransaction.expiresAt) < new Date()
      ) {
        try {
          ...
          console.error(
            "Error al actualizar estado de transacción a expirado:",
            err,
          );
          // Continuamos aunque haya error
        }
      ...
      if (transactionId && transactionId.startsWith("TR-")) {
        ...
        console.log(
          `[API Payment-Verify] Simulando verificación para plan ${isPremium ? "premium" : "standard"}`,
        );
