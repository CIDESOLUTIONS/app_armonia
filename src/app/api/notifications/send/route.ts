...
async function sendNotificationHandler(
  ...
  try {
    ...
    if (
      authPayload.role === "COMPLEX_ADMIN" &&
      authPayload.complexId !== targetComplexId
    ) {
      return NextResponse.json(
        {
          message:
            "No tienes permisos para enviar notificaciones a este complejo",
        },
        { status: 403 },
      );
    }
    ...
    // Enviar notificación
    const result =
      await pushNotificationService.sendNotification(notificationRequest);
    ...
    console.log(
      `[NOTIFICATION SENT] ${validatedData.title} enviado por ${authPayload.email} a complejo ${targetComplexId}`,
    );
