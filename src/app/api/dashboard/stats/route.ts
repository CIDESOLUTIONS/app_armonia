...
export async function GET(req: Request) {
  try {
    ...
    const [
      ...
      (await tableExists("Payment"))
        ? tenantPrisma.payment
            ...
            .then((result: any) => result._sum.amount || 0)
        : 0,
      (await tableExists("Assembly"))
        ? tenantPrisma.assembly.count({
            where: {
              date: {
                gte: new Date(),
              },
      ...
      (await tableExists("PQR"))
        ...
        : 0,
      (await tableExists("Project"))
        ? tenantPrisma.project.count({
            where: {
              status: "ACTIVE",
            },
          })
      ...
      (await tableExists("CommonArea"))
        ? tenantPrisma.commonArea.findMany({
            include: {
              availabilityConfig: true,
            },
          })
        : [],
      (await tableExists("Reservation"))
      ...
      (await tableExists("Budget"))
        ? tenantPrisma.budget.findFirst({
            where: {
              status: {
                in: ["APPROVED", "ACTIVE"],
              },
        ...
        : null,
      (await tableExists("Expense"))
        ? tenantPrisma.expense
            .aggregate({
              _sum: {
                amount: true,
              },
    ...
    if (
      allCommonAreas.length > 0 &&
      completedReservationsLastMonth.length > 0
    ) {
      let totalReservedMinutes = 0;
      completedReservationsLastMonth.forEach((res) => {
        totalReservedMinutes +=
          (res.endDateTime.getTime() - res.startDateTime.getTime()) /
          (1000 * 60);
      });
      ...
      const totalAvailableMinutes =
        allCommonAreas.length * totalPossibleMinutesPerArea;

      if (totalAvailableMinutes > 0) {
        commonAreaUsage = parseFloat(
          ((totalReservedMinutes / totalAvailableMinutes) * 100).toFixed(2),
        );
      }
    }
    ...
    if (currentBudget) {
      const totalBudgeted = currentBudget.budgetItems.reduce(
        (sum, item) => sum + item.budgetedAmount.toNumber(),
        0,
      );
      if (totalBudgeted > 0) {
        budgetExecution = parseFloat(
          ((totalExpensesCurrentBudget / totalBudgeted) * 100).toFixed(2),
        );
      }
    }
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const monthlyRevenue = (await tableExists("Payment"))
        ? await tenantPrisma.payment
            .aggregate({
              _sum: {
                amount: true,
              },
      ...
      const monthlyReservations = (await tableExists("Reservation"))
        ? await tenantPrisma.reservation.findMany({
            where: {
              status: "COMPLETED",
              createdAt: {
                gte: monthStart,
      ...
      if (allCommonAreas.length > 0) {
        const monthlyPossibleMinutesPerArea =
          12 * 60 * (monthEnd.getDate() - monthStart.getDate() + 1); // Days in month
        const monthlyTotalAvailableMinutes =
          allCommonAreas.length * monthlyPossibleMinutesPerArea;
        if (monthlyTotalAvailableMinutes > 0) {
          monthlyCommonAreaUsage = parseFloat(
            (
              (monthlyReservedMinutes / monthlyTotalAvailableMinutes) *
              100
            ).toFixed(2),
          );
        }
      }
      commonAreaUsageTrend.push({
        month: monthLabel,
        value: monthlyCommonAreaUsage,
      });
    }

    // Obtener actividad reciente del tenant
    const [recentPayments, recentPQRs, recentAssemblies, recentIncidents] =
      await Promise.all([
        (await tableExists("Payment"))
          ...
          : [],
        (await tableExists("PQR"))
          ? tenantPrisma.pQR.findMany({
              orderBy: { createdAt: "desc" },
              take: 5,
            })
          : [],
        (await tableExists("Assembly"))
          ? tenantPrisma.assembly.findMany({
              orderBy: { createdAt: "desc" },
              take: 5,
            })
          : [],
        (await tableExists("Incident"))
    ...
    const recentActivity = [
      ...
      ...recentPQRs.map((pqr) => ({
        ...
        status:
          pqr.status === "OPEN" || pqr.status === "IN_PROGRESS"
            ? "warning"
            : "info",
      })),
      ...
      ...recentIncidents.map((i) => ({
        ...
        status:
          i.status === "OPEN" || i.status === "IN_PROGRESS" ? "error" : "info",
      })),
    ];
