...
async function updateTransactionHandler(
  ...
  try {
    ...
    console.log(
      `[PAYMENTS] Transacción ${transactionId} actualizada por usuario ${payload.email} en complejo ${payload.complexId}`,
    );
  ...
  try {
    ...
    if (!["PENDING", "PROCESSING"].includes(existingTransaction.status)) {
      return NextResponse.json(
        {
          message:
            "Solo se pueden cancelar transacciones en estado PENDING o PROCESSING",
        },
        { status: 400 },
      );
    }
    ...
    console.log(
      `[PAYMENTS] Transacción ${transactionId} cancelada por usuario ${payload.email} en complejo ${payload.complexId}`,
    );
