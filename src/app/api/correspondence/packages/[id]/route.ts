...
export async function POST(
  ...
  if (isStatusRequest) {
    try {
      ...
      const sanitizedData = {
        ...
        receivedByResidentName: sanitizeInput(
          requestData.receivedByResidentName,
        ),
        signatureUrl: sanitizeInput(requestData.signatureUrl),
      };

      // Cambiar estado del paquete
      const packageData = await packageService.changePackageStatus(
        id,
        sanitizedData,
      );
  ...
  if (isDeliverRequest) {
    try {
      ...
      // Entregar paquete
      const packageData = await packageService.deliverPackage(
        id,
        sanitizedData,
      );
