...
const BudgetUpdateSchema = z.object({
  year: z
    .number()
    .int()
    .min(2000, "El año debe ser un número válido.")
    .optional(),
  status: z
...
export async function GET(
  ...
  try {
    ...
    ServerLogger.info(
      `Presupuesto ${budgetId} obtenido para el complejo ${payload.complexId}`,
    );
...
export async function PUT(
  ...
  try {
    ...
    ServerLogger.info(
      `Presupuesto ${budgetId} actualizado en complejo ${payload.complexId}`,
    );
...
export async function DELETE(
  ...
  try {
    ...
    ServerLogger.info(
      `Presupuesto ${budgetId} eliminado del complejo ${payload.complexId}`,
    );
    return NextResponse.json(
