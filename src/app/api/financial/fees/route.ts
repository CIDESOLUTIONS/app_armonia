...
const BulkFeeCreateSchema = z.object({
  ...
  endDate: z
    ...
    .regex(/^\d{4}-\d{2}-\d{2}$/, "Formato de fecha inválido (YYYY-MM-DD)."),
  unitIds: z
    .array(z.number().int().positive("ID de unidad inválido."))
    .min(1, "Se requiere al menos un ID de unidad."),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    ServerLogger.info(
      `Cuotas masivas creadas para el complejo ${payload.complexId}`,
    );
    return NextResponse.json(
      { message: "Cuotas creadas exitosamente" },
      { status: 201 },
    );
  } catch (error) {
    if (error instanceof z.ZodError) {
