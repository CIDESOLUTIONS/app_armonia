...
const UpdateLogSchema = z.object({
  status: z
    .enum(["OPEN", "IN_REVIEW", "RESOLVED", "CLOSED", "CANCELLED"])
    .optional(),
...
export async function GET(
  ...
  try {
    ...
    if (digitalLog.complexId !== payload.complexId) {
      return NextResponse.json(
        { message: "Sin acceso a esta minuta" },
        { status: 403 },
      );
    }
    ...
    return NextResponse.json(
      { message: "Error obteniendo minuta" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (!["ADMIN", "COMPLEX_ADMIN"].includes(payload.role)) {
      return NextResponse.json(
        { message: "Solo administradores pueden eliminar minutas" },
        { status: 403 },
      );
    }
    ...
    console.log(
      `[DIGITAL LOG DELETE] Minuta ${logId} eliminada por ${payload.email}`,
    );
    ...
    return NextResponse.json(
      { message: "Error eliminando minuta" },
      { status: 500 },
    );
  }
}
