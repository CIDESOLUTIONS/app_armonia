...
const CameraSchema = z.object({
  ...
  ipAddress: z
    .string()
    .ip({ version: "v4", message: "Dirección IP inválida." }),
...
export async function POST(request: NextRequest) {
  try {
    ...
    ServerLogger.info(
      `Cámara creada: ${newCamera.name} en complejo ${payload.complexId}`,
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { message: "ID de cámara requerido para actualizar" },
        { status: 400 },
      );
    }
    ...
    ServerLogger.info(
      `Cámara actualizada: ${updatedCamera.name} en complejo ${payload.complexId}`,
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { message: "ID de cámara requerido para eliminar" },
        { status: 400 },
      );
    }
    ...
    ServerLogger.info(
      `Cámara eliminada: ID ${id} en complejo ${payload.complexId}`,
    );
    return NextResponse.json(
