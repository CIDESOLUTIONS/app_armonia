...
export default function ReceptionDashboard() {
  const {
    ...
    token,
    schemaName,
    user: { name: userName, language, themeMode } = {},
  } = useAuthStore();
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const result = await getReceptionDashboardData();
      setData(result);
    } catch (err: any) {
        ...
        <div>
          <h1 className="text-2xl font-bold">
            {language === "Español"
              ...
              : "Control Panel - Reception"}
          </h1>
          <p className="text-gray-500 dark:text-gray-400">
            {language === "Español" ? "Bienvenido, " : "Welcome, "}
            {userName || "Usuario"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="md:col-span-2">
          <Card className="h-full dark:border-gray-700">
            <CardHeader className="pb-2">
              ...
              <CardDescription>
                {language === "Español"
                  ? "Personas actualmente dentro del conjunto"
                  : "People currently inside the complex"}
              </CardDescription>
              <div className="mt-2">
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="dark:border-gray-700">
                      ...
                      <TableHead className="dark:text-gray-300">
                        {language === "Español" ? "Destino" : "Destination"}
                      </TableHead>
                      <TableHead className="dark:text-gray-300">
                        {language === "Español" ? "Ingreso" : "Check-in"}
                      </TableHead>
                      <TableHead className="dark:text-gray-300">
                        {language === "Español" ? "Tiempo" : "Time"}
                      </TableHead>
                      <TableHead className="dark:text-gray-300">
                        {language === "Español" ? "Tipo" : "Type"}
                      </TableHead>
                      <TableHead className="text-right dark:text-gray-300">
                        {language === "Español" ? "Acción" : "Action"}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredVisitors.map((visitor) => (
                      ...
                      >
                        ...
                        <TableCell className="dark:text-gray-300">
                          {getElapsedTime(visitor.checkInTime)}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            onClick={() =>
                              router.push(
                                `/reception/visitors/check-out?id=${visitor.id}`,
                              )
                            }
                          >
              ...
              {data.pendingPackages.length > 0 ? (
                <div className="space-y-4">
                  {data.pendingPackages.slice(0, 3).map((pkg) => (
                    <div
                      key={pkg.id}
                      className="flex items-start border-b pb-3 last:border-0 last:pb-0 dark:border-gray-700"
                    >
              ...
              {data.recentIncidents.length > 0 ? (
                <div className="space-y-4">
                  {data.recentIncidents.map((incident) => (
                    ...
                    >
                      <div
                        className={`p-2 rounded-md mr-3 ${getSeverityColor(incident.severity)}`}
                      >
        ...
        <Card className="dark:border-gray-700">
          <CardHeader>
            ...
            <CardDescription>
              {language === "Español"
                ? "Reservas y actividades programadas para hoy"
                : "Reservations and activities scheduled for today"}
            </CardDescription>
          </CardHeader>
          <CardContent>
