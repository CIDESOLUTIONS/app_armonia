...
describe("PQR System Performance Tests", () => {
  ...
  describe("Pruebas de rendimiento para operaciones individuales", () => {
    it("debe procesar un PQR dentro del tiempo máximo aceptable", async () => {
      ...
      const pqrData = {
        ...
        description:
          "Hay una fuga de agua importante en el baño del primer piso",
        userId: 1,
        userName: "Juan Pérez",
        userRole: "RESIDENT",
        unitId: 101,
    ...
    it("debe enviar notificaciones dentro del tiempo máximo aceptable", async () => {
      ...
      expect(processingTime).toBeLessThan(
        TEST_CONFIG.maxResponseTime.statusChange,
      );
    });
    ...
    it("debe generar métricas dentro del tiempo máximo aceptable", async () => {
      ...
      jest
        .spyOn(metricsService, "getCategoryDistribution")
        .mockResolvedValueOnce([
          { category: "MAINTENANCE", count: 15, percentage: 53.57 },
        ]);

      jest
        .spyOn(metricsService, "getPriorityDistribution")
        .mockResolvedValueOnce([
          { priority: "HIGH", count: 10, percentage: 35.71 },
        ]);

      jest
        .spyOn(metricsService, "getStatusDistribution")
        .mockResolvedValueOnce([
      ...
      expect(result).toBeDefined();
      expect(processingTime).toBeLessThan(
        TEST_CONFIG.maxResponseTime.metricsGeneration,
      );
    });
  });
  ...
  describe("Pruebas de carga para operaciones masivas", () => {
    it("debe manejar múltiples creaciones de PQR concurrentes", async () => {
      ...
      console.log(
        `- Tasa de procesamiento: ${((1000 * TEST_CONFIG.numPQRs) / totalTime).toFixed(2)} PQRs/segundo`,
      );
    });
    ...
    it("debe manejar múltiples notificaciones concurrentes", async () => {
      ...
      const promises = Array.from({ length: TEST_CONFIG.numPQRs }, (_, i) => {
        ...
        return new Promise((resolve) => {
          setTimeout(
            ...
            (i * TEST_CONFIG.creationInterval) / 2,
          ); // Intervalo más corto para notificaciones
        });
      });
      ...
      console.log(`- Tiempo total: ${totalTime.toFixed(2)} ms`);
      console.log(
        `- Tiempo promedio por notificación: ${averageTime.toFixed(2)} ms`,
      );
      console.log(
        `- Tasa de envío: ${((1000 * TEST_CONFIG.numPQRs) / totalTime).toFixed(2)} notificaciones/segundo`,
      );
    });
    ...
    it("debe manejar múltiples consultas de métricas concurrentes con diferentes filtros", async () => {
      ...
      jest
        .spyOn(metricsService, "getStatusDistribution")
        .mockResolvedValue([{ status: "OPEN", count: 5, percentage: 17.86 }]);
      ...
      const promises = Array.from({ length: TEST_CONFIG.numPQRs }, (_, i) => {
        ...
        return new Promise((resolve) => {
          setTimeout(
            ...
            (i * TEST_CONFIG.creationInterval) / 4,
          ); // Intervalo más corto para consultas
        });
      });
      ...
      // Registrar métricas de rendimiento
      console.log(
        `Generación de ${TEST_CONFIG.numPQRs} dashboards de métricas:`,
      );
      console.log(`- Tiempo total: ${totalTime.toFixed(2)} ms`);
      console.log(
        `- Tiempo promedio por dashboard: ${averageTime.toFixed(2)} ms`,
      );
      console.log(
        `- Tasa de generación: ${((1000 * TEST_CONFIG.numPQRs) / totalTime).toFixed(2)} dashboards/segundo`,
      );
    });
  });
  ...
  describe("Pruebas de estrés para límites del sistema", () => {
    it("debe manejar un pico de carga en creación de PQRs", async () => {
      ...
      const promises = Array.from(
        { length: stressTestConfig.numPQRs },
        (_, i) => {
          const pqrData = {
            ...basePqrData,
            title: `${basePqrData.title} ${i + 1}`,
      ...
      // Registrar métricas de rendimiento
      console.log(
        `Prueba de estrés - Procesamiento de ${stressTestConfig.numPQRs} PQRs:`,
      );
      console.log(`- Tiempo total: ${totalTime.toFixed(2)} ms`);
      console.log(`- Tiempo promedio por PQR: ${averageTime.toFixed(2)} ms`);
      console.log(
        `- Tasa de procesamiento: ${((1000 * stressTestConfig.numPQRs) / totalTime).toFixed(2)} PQRs/segundo`,
      );
    });
  });
});
