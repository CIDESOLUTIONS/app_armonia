  ...
  describe("Flujo completo de PQR", () => {
    it("debe procesar un PQR desde creación hasta resolución", async () => {
      ...
      const pqrData = {
        ...
        description:
          "Hay una fuga de agua importante en el baño del primer piso que requiere atención urgente.",
        userId: 1,
        userName: "Juan Pérez",
        userRole: "RESIDENT",
        unitId: 101,
      ...
      const surveyResult =
        await notificationService.sendSatisfactionSurvey(123);
  ...
  describe("Escenarios de integración avanzados", () => {
    it("debe manejar correctamente la reapertura y escalamiento de un PQR", async () => {
      ...
      const resolvedPQR = {
        ...
        description:
          "La cerradura de la puerta principal no funciona correctamente",
        userId: 2,
    ...
    it("debe generar correctamente métricas de SLA y rendimiento", async () => {
      ...
      const categoryDistribution =
        await metricsService.getCategoryDistribution();
      ...
      const priorityDistribution =
        await metricsService.getPriorityDistribution();
      ...
      jest
        .spyOn(metricsService, "getSummaryMetrics")
        .mockResolvedValueOnce(summaryMetrics);
      jest
        .spyOn(metricsService, "getCategoryDistribution")
        .mockResolvedValueOnce(categoryDistribution);
      jest
        .spyOn(metricsService, "getPriorityDistribution")
        .mockResolvedValueOnce(priorityDistribution);
      jest
        .spyOn(metricsService, "getStatusDistribution")
        .mockResolvedValueOnce([]);
      jest
        .spyOn(metricsService, "getTimeTrend")
        .mockResolvedValueOnce(timeTrend);
      jest
        .spyOn(metricsService, "getSLAMetrics")
        .mockResolvedValueOnce(slaMetrics);
