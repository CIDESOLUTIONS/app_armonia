...
describe("CameraServiceONVIF", () => {
  ...
  describe("Stream Management", () => {
    ...
    it("should fail when camera not connected", async () => {
      await expect(service.getStreamProfiles(999)).rejects.toThrow(
        "Cámara no conectada",
      );
      await expect(service.getStreamUri(999)).rejects.toThrow(
        "Cámara no conectada",
      );
      await expect(service.captureSnapshot(999)).rejects.toThrow(
        "Cámara no conectada",
      );
    });
  });
  ...
  describe("PTZ Control", () => {
    ...
    it("should control PTZ movement", async () => {
      const result = await service.controlPTZ(2, "move", {
        x: 0.5,
        y: 0.3,
        z: 0.1,
      });
    ...
    it("should goto preset", async () => {
      const result = await service.controlPTZ(2, "preset", {
    ...
    it("should fail on non-PTZ camera", async () => {
      ...
      await expect(service.controlPTZ(1, "move", {})).rejects.toThrow(
        "Cámara no soporta PTZ",
      );
    });
  });
