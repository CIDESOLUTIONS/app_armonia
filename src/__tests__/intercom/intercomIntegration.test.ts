...
jest.mock("axios", () => ({
  default: {
    post: jest
      .fn()
      .mockResolvedValue({ data: { ok: true, result: { message_id: 123 } } }),
  },
}));
...
describe("IntercomService - Pruebas de integración", () => {
  ...
  describe("Flujo completo de visita", () => {
    it("debe manejar el flujo completo desde registro hasta salida", async () => {
      ...
      prisma.userIntercomPreference.findUnique = jest
        .fn()
        .mockResolvedValue(mockPreference);
      prisma.virtualIntercomNotification.create = jest
        .fn()
        .mockResolvedValue(mockNotification);
      prisma.intercomSettings.findFirst = jest
        .fn()
      ...
      prisma.userIntercomPreference.findFirst = jest
        .fn()
        .mockResolvedValue(mockPreference);
      prisma.virtualIntercomNotification.findFirst = jest
        .fn()
        .mockResolvedValue(mockNotification);
      prisma.virtualIntercomNotification.findUnique = jest
        .fn()
        .mockResolvedValue({
          ...mockNotification,
          visit: mockVisit,
  ...
  describe("Escenarios de error", () => {
    ...
    it("debe manejar errores en la notificación a residentes", async () => {
      // Configurar mock para simular error
      prisma.visit.findUnique = jest
        .fn()
        .mockRejectedValue(new Error("Error al buscar visita"));

      // Verificar que se captura la excepción
      await expect(
        intercomService.notifyResidents("visit-123"),
      ).rejects.toThrow("Error al buscar visita");
    });
    ...
    it("debe manejar errores en el procesamiento de webhooks", async () => {
      ...
      jest
        .spyOn(intercomService as any, "adapters")
        .mockReturnValue(
          new Map([[NotificationChannel.WHATSAPP, mockAdapter]]),
        );
