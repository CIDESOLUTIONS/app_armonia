...
export function initializeWebSocketServer(
  ...
  try {
    ...
    wss.on("connection", (ws: WebSocket, req: any) => {
      ...
      logger.info(
        `Cliente WebSocket conectado: ${clientId} (Schema: ${schemaName})`,
      );
      ...
      ws.on("message", (message: string) => {
        try {
          ...
          logger.debug(
            `Mensaje recibido de ${clientId}: ${JSON.stringify(parsedMessage)}`,
          );
      ...
      ws.on("error", (error: Error) => {
        logger.error(
          `Error en conexión WebSocket (${clientId}): ${error.message}`,
        );
      });
    });
...
export function sendToClient(
  ...
  try {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      ...
      logger.debug(
        `Mensaje enviado a cliente ${clientId}: ${JSON.stringify(data)}`,
      );
      return true;
    } else {
