...
const complexData = {
  ...
  commonAreas:
    "Salón Social, Gimnasio, Piscina, Zona BBQ, Parque Infantil, Cancha de Tenis",
};
...
test.describe("Inventario - Gestión Completa del Conjunto", () => {
  ...
  test("Flujo completo: Actualizar información del conjunto y gestionar inventario", async ({
    page,
  }) => {
    ...
    await test.step("Navegar al módulo de inventario", async () => {
      const inventoryLink = page
        .locator(
          'a:has-text("Inventario"), a:has-text("Gestión"), [href*="inventory"]',
        )
        .first();
    ...
    await test.step("Actualizar información del conjunto", async () => {
      ...
      const complexInfoTab = page
        .locator(
          'tab:has-text("Conjunto"), button:has-text("Información"), a:has-text("Datos")',
        )
        .first();
      ...
      const editButton = page
        .locator(
          'button:has-text("Editar"), button:has-text("Modificar"), .edit-complex-info',
        )
        .first();
      ...
      await page.fill(
        'input[name="address"], #complex-address',
        complexData.address,
      );
      ...
      await page.fill(
        'input[name="website"], #complex-website',
        complexData.website,
      );
      await page.fill('input[name="units"], #total-units', complexData.units);
      await page.fill(
        'input[name="towers"], #total-towers',
        complexData.towers,
      );
      await page.fill(
        'input[name="parkingSpots"], #parking-spots',
        complexData.parkingSpots,
      );
      // Descripción y áreas comunes
      const descriptionField = page
        .locator('textarea[name="description"], #complex-description')
        .first();
      if (await descriptionField.isVisible()) {
        await descriptionField.fill(complexData.description);
      }
      const commonAreasField = page
        .locator('textarea[name="commonAreas"], #common-areas')
        .first();
      ...
      await expect(
        page.locator("text=/actualizada/i, text=/guardada/i, text=/éxito/i"),
      ).toBeVisible({ timeout: 15000 });
    });
    // PASO 4: Gestionar residentes
    await test.step("Agregar nuevos residentes", async () => {
      // Navegar a sección de residentes
      const residentsTab = page
        .locator(
          'tab:has-text("Residentes"), button:has-text("Residentes"), a:has-text("Residentes")',
        )
        .first();
      ...
      for (const resident of newResidents) {
        ...
        await page.fill(
          'input[name="unitNumber"], #unit-number',
          resident.unitNumber,
        );
        // Seleccionar tipo de residente
        const residentTypeSelect = page
          .locator('select[name="residentType"], #resident-type')
          .first();
        if (await residentTypeSelect.isVisible()) {
          await residentTypeSelect.selectOption(resident.residentType);
        }
        // Información adicional
        const emergencyContactField = page
          .locator('input[name="emergencyContact"], #emergency-contact')
          .first();
        if (await emergencyContactField.isVisible()) {
          await emergencyContactField.fill(resident.emergencyContact);
        }
        const moveInDateField = page
          .locator(
            'input[name="moveInDate"], input[type="date"], #move-in-date',
          )
          .first();
        ...
        // Guardar residente
        const saveResidentButton = page
          .locator('button[type="submit"], button:has-text("Guardar")')
          .first();
        await saveResidentButton.click();

        await expect(
          page.locator("text=/residente.*agregado/i, text=/registrado/i"),
    ...
    await test.step("Agregar y configurar propiedades", async () => {
      ...
      const propertiesTab = page
        .locator(
          'tab:has-text("Propiedades"), button:has-text("Unidades"), a:has-text("Propiedades")',
        )
        .first();
      ...
      for (const property of properties) {
        const addPropertyButton = page
          .locator('button:has-text("Agregar"), button:has-text("Nueva")')
          .first();
        ...
        const propertyTypeSelect = page
          .locator('select[name="type"], #property-type')
          .first();
        ...
        if (property.type === "APARTMENT") {
          const bedroomsField = page
            .locator('input[name="bedrooms"], #bedrooms')
            .first();
          if (await bedroomsField.isVisible()) {
            await bedroomsField.fill(property.bedrooms);
          }
          const bathroomsField = page
            .locator('input[name="bathrooms"], #bathrooms')
            .first();
          ...
          // Balcón (checkbox)
          if (property.balcony) {
            const balconyCheckbox = page
              .locator('input[name="balcony"], #has-balcony')
              .first();
        ...
        const statusSelect = page
          .locator('select[name="status"], #property-status')
          .first();
        if (await statusSelect.isVisible()) {
          await statusSelect.selectOption(property.status);
        }
        const parkingSpotsField = page
          .locator('input[name="parkingSpots"], #parking-spots')
          .first();
        if ((await parkingSpotsField.isVisible()) && property.parkingSpots) {
          await parkingSpotsField.fill(property.parkingSpots);
        }
        // Guardar propiedad
        const savePropertyButton = page
          .locator('button[type="submit"], button:has-text("Guardar")')
          .first();
        await savePropertyButton.click();

        await expect(
          page.locator("text=/propiedad.*agregada/i, text=/registrada/i"),
        ).toBeVisible({ timeout: 10000 });
        await page.waitForTimeout(1000);
      }
    });
    ...
    await test.step("Registrar mascotas de residentes", async () => {
      ...
      const petsTab = page
        .locator(
          'tab:has-text("Mascotas"), button:has-text("Pets"), a:has-text("Mascotas")',
        )
        .first();
      await petsTab.click();
      await page.waitForLoadState("networkidle");
      ...
      for (const pet of pets) {
        const addPetButton = page
          .locator('button:has-text("Registrar"), button:has-text("Nueva")')
          .first();
        ...
        // Tipo de mascota
        const petTypeSelect = page
          .locator('select[name="petType"], #pet-type')
          .first();
        if (await petTypeSelect.isVisible()) {
          await petTypeSelect.selectOption(pet.type);
        }
        // Unidad del propietario
        const ownerUnitSelect = page
          .locator('select[name="ownerUnit"], #owner-unit')
          .first();
        ...
        // Vacunación
        if (pet.vaccinated) {
          const vaccinatedCheckbox = page
            .locator('input[name="vaccinated"], #is-vaccinated')
            .first();
        ...
        // Fecha de registro
        const registrationDateField = page
          .locator(
            'input[name="registrationDate"], input[type="date"], #registration-date',
          )
          .first();
        ...
        // Guardar mascota
        const savePetButton = page
          .locator('button[type="submit"], button:has-text("Registrar")')
          .first();
        await savePetButton.click();

        await expect(
          page.locator("text=/mascota.*registrada/i, text=/guardada/i"),
        ).toBeVisible({ timeout: 10000 });
        await page.waitForTimeout(1000);
      }
    });
    ...
    await test.step("Registrar vehículos de residentes", async () => {
      // Navegar a vehículos
      const vehiclesTab = page
        .locator(
          'tab:has-text("Vehículos"), button:has-text("Vehicles"), a:has-text("Vehículos")',
        )
        .first();
      ...
      for (const vehicle of vehicles) {
        const addVehicleButton = page
          .locator('button:has-text("Registrar"), button:has-text("Nuevo")')
          .first();
        ...
        // Tipo de vehículo
        const vehicleTypeSelect = page
          .locator('select[name="vehicleType"], #vehicle-type')
          .first();
        ...
        const parkingSpotField = page
          .locator('input[name="parkingSpot"], #parking-spot')
          .first();
        if (await parkingSpotField.isVisible()) {
          await parkingSpotField.fill(vehicle.parkingSpot);
        }
        // Guardar vehículo
        const saveVehicleButton = page
          .locator('button[type="submit"], button:has-text("Registrar")')
          .first();
        await saveVehicleButton.click();

        await expect(
          page.locator("text=/vehículo.*registrado/i, text=/guardado/i"),
        ).toBeVisible({ timeout: 10000 });
        await page.waitForTimeout(1000);
      }
    });
    ...
    await test.step("Verificar estadísticas y resumen del inventario", async () => {
      ...
      const summaryTab = page
        .locator(
          'tab:has-text("Resumen"), button:has-text("Estadísticas"), a:has-text("Dashboard")',
        )
        .first();
      if (await summaryTab.isVisible()) {
        ...
        await expect(
          page.locator(`text="${complexData.units}", text="52"`),
        ).toBeVisible();
        await expect(
          page.locator("text=/residentes.*registrados/i"),
        ).toBeVisible();
        await expect(
          page.locator("text=/propiedades.*registradas/i"),
        ).toBeVisible();
        await expect(
          page.locator("text=/mascotas.*registradas/i"),
        ).toBeVisible();
        await expect(
          page.locator("text=/vehículos.*registrados/i"),
        ).toBeVisible();
      }
    });
  });
  test("Buscar y filtrar en el inventario", async ({ page }) => {
    ...
    const searchInput = page
      .locator('input[name="search"], #search-residents')
      .first();
    ...
    const residentTypeFilter = page
      .locator('select[name="residentType"], #filter-resident-type')
      .first();
    if (await residentTypeFilter.isVisible()) {
      ...
      // Verificar filtrado
      await expect(
        page.locator("text=/propietario/i, text=/owner/i"),
      ).toBeVisible();
    }
    // Filtrar por torre
    const towerFilter = page
      .locator('select[name="tower"], #filter-tower')
      .first();
  ...
  test("Exportar reportes de inventario", async ({ page }) => {
    ...
    const exportButton = page
      .locator(
        'button:has-text("Exportar"), button:has-text("Descargar"), .export-button',
      )
      .first();
    if (await exportButton.isVisible()) {
      ...
      const reportTypeSelect = page
        .locator('select[name="reportType"], #report-type')
        .first();
      ...
      const formatSelect = page
        .locator('select[name="format"], #export-format')
        .first();
      ...
      const generateButton = page
        .locator('button:has-text("Generar"), button:has-text("Crear")')
        .first();
      if (await generateButton.isVisible()) {
        ...
        await expect(
          page.locator("text=/reporte.*generado/i, text=/descarga/i"),
        ).toBeVisible({ timeout: 15000 });
      }
    }
  });
  test("Gestión de servicios comunes", async ({ page }) => {
    ...
    await page
      .locator('a:has-text("Servicios"), a:has-text("Common Services")')
      .first()
      .click();
    await page.waitForLoadState("networkidle");
    // Configurar nuevo servicio común
    const addServiceButton = page
      .locator('button:has-text("Agregar"), button:has-text("Nuevo Servicio")')
      .first();
    if (await addServiceButton.isVisible()) {
      ...
      await page.fill(
        ...
        "Servicio de Limpieza Extraordinaria",
      );
      await page.fill(
        'textarea[name="description"], #service-description',
        "Servicio de limpieza profunda de áreas comunes",
      );
      await page.fill('input[name="cost"], #service-cost', "150000");

      // Tipo de servicio
      const serviceTypeSelect = page
        .locator('select[name="serviceType"], #service-type')
        .first();
      ...
      const frequencySelect = page
        .locator('select[name="frequency"], #service-frequency')
        .first();
      ...
      await expect(
        page.locator("text=/servicio.*agregado/i, text=/configurado/i"),
      ).toBeVisible({ timeout: 10000 });
    }
  });
});
