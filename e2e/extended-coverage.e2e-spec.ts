...
test.describe("Extended Coverage E2E Tests", () => {
  ...
  test("CP-212: should manage annual budget", async ({ page }) => {
    ...
    await expect(page.locator("text=Presupuesto creado con éxito")).toBeVisible();
  ...
  test("CP-218: should create and analyze surveys", async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', "Opine sobre nuestros servicios.");
    await page.fill('input[name="endDate"]', "2025-12-31");
    // Add a question
    await page.click('button:has-text("Añadir Pregunta")');
    await page.fill('input[name="questions.0.text"]', "¿Qué tan satisfecho está con la limpieza?");
  ...
  test("CP-220: should moderate reported marketplace ads", async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', "Contenido para ser revisado por admin.");
    ...
    await page.locator(`tr:has-text("${adTitle}") button:has-text("Eliminar")`).click();
    page.on('dialog', dialog => dialog.accept());
  ...
  test("CP-310: should allow resident to create and track PQR", async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', "Esta es una PQR de prueba desde el portal de residente.");
    ...
    await page.locator(`tr:has-text("${pqrSubject}") button:has-text("Ver")`).click();
    ...
    await expect(page.locator(`tr:has-text("${pqrSubject}") td:has-text("En Progreso")`)).toBeVisible();
  });
});
