...
const testPQRs = {
  ...
  security: {
    ...
    description:
      "La cámara de seguridad del parqueadero no está funcionando desde ayer.",
    type: "COMPLAINT",
...
class PQRPage {
  ...
  async verifyPQRExists(ticketNumber: string) {
    ...
    await expect(
      this.page.locator(`tr:has-text("${ticketNumber}")`),
    ).toBeVisible();
  }
  ...
  async verifyMetricsDashboard() {
    ...
    await expect(
      this.page.locator('div.card:has-text("Resumen")'),
    ).toBeVisible();
    await expect(this.page.locator("div.chart-container")).toHaveCount(4);
    await expect(
      this.page.locator('table:has-text("Cumplimiento de SLA")'),
    ).toBeVisible();
  }
}
...
test.describe("Sistema PQR - Flujos E2E", () => {
  ...
  test("Flujo completo: Creación, asignación y resolución de PQR", async ({
    page,
  }) => {
    pqrPage = new PQRPage(page);
    ...
    await expect(
      page.locator('h1:has-text("Dashboard de Residente")'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('h1:has-text("Dashboard de Administración")'),
    ).toBeVisible();
    ...
    await pqrPage.ratePQR(
      ticketNumber,
      5,
      "Excelente servicio, rápido y efectivo",
    );
  ...
  test("Flujo de reapertura: Resolución, reapertura y resolución final", async ({
    page,
  }) => {
    pqrPage = new PQRPage(page);
  ...
  test("Prueba de responsividad en dispositivos móviles", async ({ page }) => {
    ...
    await page.fill(
      ...
      testPQRs.maintenance.description,
    );
