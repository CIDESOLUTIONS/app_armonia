...
const testIncidents = [
  {
    ...
    description:
      "Ronda rutinaria completada. Todas las áreas revisadas sin novedad. Puertas y ventanas seguras.",
  ...
  },
  {
    ...
    description:
      "Se observó vehículo con placas ABC-123 estacionado en zona de visitantes por más de 4 horas sin autorización.",
  ...
  },
  {
    ...
    description:
      "Residente del apartamento 502 reportó fuerte olor a gas. Se evacuó el piso y se contactó empresa de gas.",
...
test.describe("Seguridad - Minutas Digitales y Control de Acceso", () => {
  ...
  test("Flujo completo: Guardia registra minuta y admin consulta", async ({
    page,
  }) => {
    ...
    await test.step("Login como guardia de día", async () => {
      ...
      await page.fill(
        ...
        testUsers.guard1.email,
      );
      await page.fill(
        'input[name="password"], input[type="password"]',
        testUsers.guard1.password,
      );
    ...
    await test.step("Registrar ronda de seguridad rutinaria", async () => {
      ...
      const securityLink = page
        .locator(
          'a:has-text("Seguridad"), a:has-text("Minutas"), [href*="security"]',
        )
        .first();
      ...
      const newIncidentButton = page
        .locator(
          'button:has-text("Nuevo"), button:has-text("Registrar"), button:has-text("Crear")',
        )
        .first();
      ...
      const typeSelect = page
        .locator('select[name="type"], #incident-type')
        .first();
      if (await typeSelect.isVisible()) {
        await typeSelect.selectOption(routineIncident.type);
      }
      await page.fill(
        'input[name="title"], #incident-title',
        routineIncident.title,
      );
      await page.fill(
        'textarea[name="description"], #incident-description',
        routineIncident.description,
      );
      await page.fill(
        'input[name="location"], #incident-location',
        routineIncident.location,
      );
      await page.fill(
        'input[name="time"], input[type="time"], #incident-time',
        routineIncident.time,
      );
      const severitySelect = page
        .locator('select[name="severity"], #incident-severity')
        .first();
      ...
      await expect(
        page.locator("text=/registrado/i, text=/guardado/i, text=/éxito/i"),
      ).toBeVisible({ timeout: 15000 });
    });
    // PASO 3: Registrar visitante
    await test.step("Registrar entrada de visitante", async () => {
      // Navegar a control de visitantes
      const visitorsLink = page
        .locator(
          'a:has-text("Visitantes"), a:has-text("Control"), [href*="visitor"]',
        )
        .first();
      if (await visitorsLink.isVisible()) {
        ...
        await page
          .locator('button:has-text("Visitantes"), .visitor-control')
          .first()
          .click();
      }
      await page.waitForLoadState("networkidle");
      // Registrar nuevo visitante
      const registerVisitorButton = page
      ...
      await page.fill(
        'input[name="visitorPhone"], #visitor-phone',
        visitor.phone,
      );
      await page.fill(
        'input[name="visitingUnit"], #visiting-unit',
        visitor.visitingUnit,
      );
      await page.fill('input[name="purpose"], #visit-purpose', visitor.purpose);
      await page.fill(
        'input[name="vehiclePlate"], #vehicle-plate',
        visitor.vehiclePlate,
      );
      // Registrar entrada
      const registerEntryButton = page
        .locator('button[type="submit"], button:has-text("Registrar Entrada")')
        .first();
      await registerEntryButton.click();
      await expect(
        page.locator(
          "text=/visitante.*registrado/i, text=/entrada.*autorizada/i",
        ),
      ).toBeVisible();
    });
    ...
    await test.step("Registrar incidente con vehículo sospechoso", async () => {
      ...
      await page
        .locator('a:has-text("Minutas"), a:has-text("Incidentes")')
        .first()
        .click();

      const newIncidentButton = page
        .locator('button:has-text("Nuevo"), button:has-text("Registrar")')
        .first();
      await newIncidentButton.click();
      ...
      const actionsField = page
        .locator('textarea[name="actions"], #incident-actions')
        .first();
    ...
    await test.step("Finalizar turno de guardia", async () => {
      ...
      const endShiftButton = page
        .locator(
          'button:has-text("Finalizar Turno"), button:has-text("Cerrar Turno")',
        )
        .first();
      if (await endShiftButton.isVisible()) {
        ...
        const confirmButton = page
          .locator('button:has-text("Confirmar"), button:has-text("Sí")')
          .first();
        ...
        await expect(
          page.locator("text=/turno.*finalizado/i, text=/minuta.*cerrada/i"),
        ).toBeVisible();
      }
    });
    ...
    await test.step("Admin consulta minutas del día", async () => {
      ...
      await expect(
        page.locator(`text="${testIncidents[0].title}"`),
      ).toBeVisible();
      await expect(
        page.locator(`text="${testIncidents[1].title}"`),
      ).toBeVisible();

      // Verificar información de guardias
      await expect(
        page.locator(`text="${testUsers.guard1.name}"`),
      ).toBeVisible();
    });
    ...
    await test.step("Ver detalle de incidente específico", async () => {
      ...
      await expect(
        page.locator(`text="${testIncidents[1].description}"`),
      ).toBeVisible();
      await expect(
        page.locator(`text="${testIncidents[1].actions}"`),
      ).toBeVisible();
    ...
    await test.step("Generar reporte de seguridad del día", async () => {
      ...
      const reportsButton = page
        .locator(
          'button:has-text("Reporte"), button:has-text("Exportar"), a:has-text("Reportes")',
        )
        .first();
      if (await reportsButton.isVisible()) {
        ...
        const todayOption = page
          .locator('option:has-text("Hoy"), input[value="today"]')
          .first();
        ...
        const generateButton = page
          .locator(
            'button:has-text("Generar"), button:has-text("Crear Reporte")',
          )
          .first();
  ...
  test("Registro de emergencia por guardia nocturno", async ({ page }) => {
    ...
    const emergencyCheckbox = page
      .locator('input[name="isEmergency"], input[type="checkbox"]')
      .first();
    ...
    await expect(
      page.locator("text=/emergencia.*registrada/i, text=/alerta.*enviada/i"),
    ).toBeVisible();
  });
  test("Control de salida de visitantes", async ({ page }) => {
    ...
    const activeVisitorsTab = page
      .locator('tab:has-text("Activos"), button:has-text("En el conjunto")')
      .first();
    ...
    const exitButton = page
      .locator(`text="${visitor.name}"~button:has-text("Salida"), .exit-button`)
      .first();
    if (await exitButton.isVisible()) {
      await exitButton.click();

      // Confirmar salida
      const confirmExitButton = page
        .locator('button:has-text("Confirmar Salida")')
        .first();
  ...
  test("Búsqueda y filtros en histórico de minutas", async ({ page }) => {
    ...
    const historyTab = page
      .locator('tab:has-text("Histórico"), a:has-text("Ver Todo")')
      .first();
    ...
    const severityFilter = page
      .locator('select[name="severity"], #severity-filter')
      .first();
    if (await severityFilter.isVisible()) {
      ...
      await expect(
        page.locator("text=/emergency/i, text=/alta/i"),
      ).toBeVisible();
    }
    // Búsqueda por texto
    const searchInput = page
      .locator('input[name="search"], #search-input')
      .first();
