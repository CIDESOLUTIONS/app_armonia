...
test.describe("Resident Portal E2E Tests", () => {
  ...
  test("CP-303: should allow amenity reservation", async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', "Reserva para pruebas E2E.");
    ...
    await page.fill('input[name="startDateTime"]', formatDateTime(startDateTime));
  ...
  test("CP-304: should allow visitor pre-registration and QR generation", async ({ page }) => {
    ...
    await expect(page.locator("text=Visitante pre-registrado correctamente. Código QR generado.")).toBeVisible();
    await expect(page.locator("img[alt=\"Código QR del visitante\"]")).toBeVisible();
  });

  // CP-305 - Botón de pánico (registro y notificación)
  test("CP-305: should trigger panic button and register alert", async ({ page }) => {
    ...
    await expect(page.locator("text=El personal de seguridad ha sido notificado.")).toBeVisible();
  });
  ...
  test("CP-307: should allow marketplace ad posting", async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', "Vendo artículo en buen estado para pruebas E2E.");
  ...
  test("CP-309: should allow content reporting", async ({ page }) => {
    ...
    await expect(page.locator("text=Gracias por tu reporte. Lo revisaremos pronto.")).toBeVisible();
  });
});