...
test.describe("Reception Portal E2E Tests", () => {
  ...
  test("CP-401: should register visitor entry with QR", async ({ page }) => {
    ...
    const qrCodeSrc = await page
      .locator('img[alt="Código QR del visitante"]')
      .getAttribute("src");
    ...
    await page.fill(
      'input[placeholder="Escanear o introducir código QR"]',
      qrCodeData,
    );
    ...
    await expect(
      page.locator("text=QR Code escaneado con éxito."),
    ).toBeVisible();
  ...
  test("CP-402: should register visitor manually", async ({ page }) => {
    ...
    await page
      .locator('div[role="option"]:has-text("Cédula de Ciudadanía")')
      .click(); // Select CC
  ...
  test("CP-403: should register package and notify resident", async ({
    page,
  }) => {
    await login(page, receptionEmail, receptionPassword, "reception");
  ...
  test("CP-404: should log new incident", async ({ page }) => {
    ...
    await page.fill(
      ...
      "Descripción del incidente de prueba E2E.",
    );
  ...
  test("CP-405: should receive and respond to panic alert", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=Alerta resuelta correctamente."),
    ).toBeVisible();
  });
});
