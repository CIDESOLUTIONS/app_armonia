...
async function login(
  ...
  if (portal === "admin") {
    ...
    await page.locator('[data-testid="portal-reception-button"]').click();
  }
...
test.describe("Armonía Application E2E Tests", () => {
  ...
  test("CP-201: should manage properties (CRUD)", async ({ page }) => {
    // Navegación natural a través de la UI
    await page.locator('nav >> button:has-text("Inventario")').click();
    await page.locator('nav >> button:has-text("Inmuebles")').click();

    // Verificar que la navegación fue exitosa
    await expect(page).toHaveURL(/.*\/inventory\/properties/);
    ...
    await page.click(
      'button:has-text("Añadir"), button:has-text("Add"), button:has-text("Crear")',
    );
    ...
    await expect(
      page.locator("text=creado, text=created, text=éxito, text=success"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=actualizado, text=updated, text=éxito, text=success"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=eliminado, text=deleted, text=éxito, text=success"),
    ).toBeVisible();
  });
  ...
  test("CP-207: should create and publish announcements", async ({ page }) => {
    ...
    await page.click(
      'button:has-text("Crear"), button:has-text("Add"), button:has-text("Nuevo")',
    );
    ...
    await page.click(
      'button:has-text("Publicar"), button:has-text("Publish"), button[type="submit"]',
    );
    await expect(
      page.locator("text=publicado, text=published, text=éxito, text=success"),
    ).toBeVisible();
  });
  ...
  test("CP-208: should send notifications", async ({ page }) => {
    // Navegación natural a través de la UI
    // NOTA: No se encontró "Comunicaciones" en el Sidebar, se asume que está en otra parte o es un error.
    // Por ahora, se mantiene el goto, pero se marca para revisión.
    ...
    await expect(
      page.locator("text=enviada, text=sent, text=éxito, text=success"),
    ).toBeVisible();
  });
  ...
  test("CP-209: should manage PQRs (CRUD, comments, assign)", async ({
    ...
    await page.click(
      'button:has-text("Crear"), button:has-text("Add"), button:has-text("Nueva")',
    );
    ...
    await page.click(
      'button:has-text("Crear"), button:has-text("Save"), button[type="submit"]',
    );
    await expect(
      page.locator("text=creada, text=created, text=éxito, text=success"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=añadido, text=added, text=éxito, text=success"),
    ).toBeVisible();
    ...
    await page.click(
      'button:has-text("Asignar"), button:has-text("Assign"), button[type="submit"]',
    );
    await expect(
      page.locator("text=asignada, text=assigned, text=éxito, text=success"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=actualizada, text=updated, text=éxito, text=success"),
    ).toBeVisible();
  });
  ...
  test("CP-210: should generate ordinary fees", async ({ page }) => {
    ...
    await expect(
      page.locator("text=generadas, text=generated, text=éxito, text=success"),
    ).toBeVisible();
  });
  ...
  test("CP-211: should register manual payments", async ({ page }) => {
    ...
    await page.click(
      'button:has-text("Registrar"), button:has-text("Register"), button:has-text("Manual")',
    );
    ...
    await page.click(
      'button:has-text("Registrar"), button:has-text("Register"), button[type="submit"]',
    );
    await expect(
      page.locator(
        "text=registrado, text=registered, text=éxito, text=success",
      ),
    ).toBeVisible();
  });
  ...
  test("CP-213: should generate peace and safe report", async ({ page }) => {
    ...
    await page.click('button:has-text("Generar"), button:has-text("Generate")');
    await expect(
      page.locator("text=generado, text=generated, text=éxito, text=success"),
    ).toBeVisible();
  });

  // CP-214 - Conciliación Bancaria Automática
  test("CP-214: should perform bank reconciliation with file upload", async ({
    page,
  }) => {
    // No se encontró un enlace directo para Conciliación en el Sidebar.
    // Se asume que esta acción se realiza desde otra página de Finanzas.
    // Se elimina el goto y se confía en que el contexto es correcto.
    await page.waitForLoadState("networkidle");
    ...
    await expect(
      page.locator("text=Archivo procesado y conciliado"),
    ).toBeVisible();
  ...
  test("CP-217: should manage staff", async ({ page }) => {
    // Navegación natural a través de la UI
    await page.locator('nav >> button:has-text("Usuarios")').click();
    await page.locator('nav >> button:has-text("Servicios Generales")').click();
...
test.describe("Portal de Recepción", () => {
  test.beforeEach(async ({ page }) => {
    const receptionEmail =
      process.env.E2E_RECEPTION_EMAIL || `reception.e2e.${Date.now()}@test.com`;
    const receptionPassword =
      process.env.E2E_RECEPTION_PASSWORD || "password123";
