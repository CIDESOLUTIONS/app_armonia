...
test.describe("Navegación Completa - Landing a Dashboard", () => {
  ...
  test("Flujo completo: Landing → Portal Administrador → Dashboard completo", async ({
    page,
  }) => {
    ...
    await test.step("Verificar elementos de la landing page", async () => {
      ...
      await expect(
        page.locator("text=/gestión/i, text=/conjuntos/i, text=/residencial/i"),
      ).toBeVisible();

      // Verificar botones de acción
      const ctaButtons = page.locator(
        'button:has-text("Comenzar"), a:has-text("Iniciar"), button:has-text("Probar")',
      );
    ...
    await test.step("Navegar al selector de portales", async () => {
      ...
      const accessButton = page
        .locator(
          'button:has-text("Acceder"), a:has-text("Login"), button:has-text("Ingresar"), a:has-text("Portal")',
        )
        .first();
      ...
      await expect(
        page.locator("text=/selecciona.*portal/i, text=/tipo.*usuario/i"),
      ).toBeVisible();
    });
    ...
    await test.step("Seleccionar portal de Administrador", async () => {
      ...
      const adminPortal = page
        .locator(
          'button:has-text("Administrador"), .admin-portal, [data-portal="admin"]',
        )
        .first();
    ...
    await test.step("Login como Administrador", async () => {
      ...
      const loginButton = page
        .locator(
          'button[type="submit"], button:has-text("Iniciar"), button:has-text("Ingresar")',
        )
        .first();
      ...
      await expect(
        page.locator(`text="${testUsers.admin.name}", text=/bienvenido/i`),
    ...
    await test.step("Verificar estructura del dashboard de administrador", async () => {
      ...
      const sidebar = page
        .locator('.sidebar, #sidebar, nav[role="navigation"]')
        .first();
      await expect(sidebar).toBeVisible();

      // Verificar que el menú es colapsable
      const toggleButton = page
        .locator('button[aria-label*="toggle"], .menu-toggle, .sidebar-toggle')
        .first();
      ...
      await expect(
        page.locator("main, .main-content, .dashboard-content"),
      ).toBeVisible();
    });
    ...
    await test.step("Navegar por todas las opciones del menú lateral", async () => {
      for (const menuItem of menuItems.admin) {
        ...
        const menuLink = page
          .locator(
            `a:has-text("${menuItem.name}"), [href*="${menuItem.href}"], .menu-item:has-text("${menuItem.name}")`,
          )
          .first();
        ...
        if (await menuLink.isVisible()) {
          ...
          await expect(
            page.locator(
              `h1:has-text("${menuItem.name}"), .page-title:has-text("${menuItem.name}"), h2:has-text("${menuItem.name}")`,
            ),
    ...
    await test.step("Verificar funcionalidades específicas de módulos clave", async () => {
      ...
      await expect(
        page.locator("text=/estadísticas/i, .stats, .metrics"),
      ).toBeVisible();

      // Inventario
      await page.locator('a:has-text("Inventario")').first().click();
      await expect(
        page.locator("text=/residentes/i, text=/propiedades/i"),
      ).toBeVisible();

      // Finanzas
      await page.locator('a:has-text("Finanzas")').first().click();
      await expect(
        page.locator("text=/facturación/i, text=/pagos/i, text=/ingresos/i"),
      ).toBeVisible();
      ...
      if (await assemblyLink.isVisible()) {
        ...
        await expect(
          page.locator("text=/votación/i, text=/asamblea/i"),
        ).toBeVisible();
      }
      ...
      await expect(
        page.locator("text=/peticiones/i, text=/quejas/i, text=/reclamos/i"),
      ).toBeVisible();
      ...
      if (await securityLink.isVisible()) {
        ...
        await expect(
          page.locator("text=/acceso/i, text=/control/i, text=/visitantes/i"),
        ).toBeVisible();
      }
    });
    ...
    await test.step("Verificar funcionalidades responsivas del menú", async () => {
      ...
      const menuToggle = page
        .locator('.menu-toggle, .sidebar-toggle, button[aria-label*="menu"]')
        .first();
      if (await menuToggle.isVisible()) {
        ...
        const collapsedSidebar = page
          .locator(".sidebar-collapsed, .sidebar.collapsed, nav.collapsed")
          .first();
      ...
      const tooltipTrigger = page
        .locator(".menu-item[title], .nav-item[data-tooltip]")
        .first();
  ...
  test("Flujo completo: Landing → Portal Residente → Dashboard residente", async ({
    page,
  }) => {
    // PASO 1: Navegar desde landing al portal residente
    await test.step("Navegar al portal de residente", async () => {
      const accessButton = page
        .locator('button:has-text("Acceder"), a:has-text("Portal")')
        .first();
      await accessButton.click();
      await page.waitForLoadState("networkidle");
      // Seleccionar portal residente
      const residentPortal = page
        .locator(
          'button:has-text("Residente"), .resident-portal, [data-portal="resident"]',
        )
        .first();
    ...
    await test.step("Login como residente", async () => {
      ...
      await expect(
        page.locator(`text="${testUsers.resident.name}", text=/residente/i`),
      ).toBeVisible();
    });
    ...
    await test.step("Navegar por opciones del menú de residente", async () => {
      for (const menuItem of menuItems.resident) {
        ...
        if (await menuLink.isVisible()) {
          ...
          await expect(
            page.locator(`h1:has-text("${menuItem.name}"), .page-title`),
          ).toBeVisible({ timeout: 10000 });
    ...
    await test.step("Verificar funcionalidades específicas de residente", async () => {
      ...
      await expect(
        page.locator("text=/información personal/i, text=/datos/i"),
      ).toBeVisible();

      // Pagos
      await page.locator('a:has-text("Pagos")').first().click();
      await expect(
        page.locator("text=/estado de cuenta/i, text=/facturas/i"),
      ).toBeVisible();

      // Reservas
      await page.locator('a:has-text("Reservas")').first().click();
      await expect(
        page.locator("text=/áreas comunes/i, text=/salón/i, text=/gimnasio/i"),
      ).toBeVisible();
    });
  });
  test("Flujo completo: Landing → Portal Recepción → Dashboard recepción", async ({
    page,
  }) => {
    ...
    await test.step("Navegar al portal de recepción", async () => {
      ...
      const receptionPortal = page
        .locator(
          'button:has-text("Recepción"), .reception-portal, [data-portal="reception"]',
        )
        .first();
    ...
    await test.step("Login como recepcionista", async () => {
      ...
      await expect(
        page.locator(`text="${testUsers.reception.name}", text=/recepción/i`),
      ).toBeVisible();
    });
    ...
    await test.step("Verificar funcionalidades específicas de recepción", async () => {
      ...
      await expect(
        page.locator("text=/registro de visitas/i, text=/autorización/i"),
      ).toBeVisible();

      // Paquetes
      await page.locator('a:has-text("Paquetes")').first().click();
      await expect(
        page.locator("text=/correspondencia/i, text=/entrega/i"),
      ).toBeVisible();

      // Incidentes
      await page.locator('a:has-text("Incidentes")').first().click();
      await expect(
        page.locator("text=/reportar/i, text=/novedad/i"),
      ).toBeVisible();
    });
  });
  test("Verificar accesibilidad y UX del menú lateral", async ({ page }) => {
    ...
    await test.step("Verificar estados activo/inactivo del menú", async () => {
      ...
      await expect(
        page.locator(
          'a:has-text("Inventario").active, .menu-item.active:has-text("Inventario")',
        ),
      ).toBeVisible();
      ...
      await expect(
        page.locator(
          'a:has-text("Finanzas").active, .menu-item.active:has-text("Finanzas")',
        ),
      ).toBeVisible();
    });
  ...
  test("Verificar logout y cambio de portales", async ({ page }) => {
    ...
    await test.step("Verificar logout desde dashboard", async () => {
      ...
      const logoutButton = page
        .locator(
          'button:has-text("Salir"), button:has-text("Logout"), .logout-btn',
        )
        .first();
