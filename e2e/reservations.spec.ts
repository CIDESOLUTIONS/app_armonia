...
test.describe("Reservas - Áreas Comunes", () => {
  ...
  test("Flujo completo: Consultar disponibilidad, reservar y pagar área común", async ({
    page,
  }) => {
    ...
    await test.step("Login como residente", async () => {
      ...
      await page.fill(
        ...
        testUsers.resident1.email,
      );
      await page.fill(
        'input[name="password"], input[type="password"]',
        testUsers.resident1.password,
      );
    ...
    await test.step("Navegar al módulo de reservas", async () => {
      ...
      const reservationsLink = page
        .locator(
          'a:has-text("Reservas"), a:has-text("Áreas Comunes"), [href*="reserv"]',
        )
        .first();
    ...
    await test.step("Consultar áreas comunes disponibles", async () => {
      ...
      const salonSocialCard = page
        .locator(`text="${testReservation.area}"`)
        .first();
      await salonSocialCard.click();
      // Verificar detalles del área
      await expect(
        page.locator(`text="${testCommonAreas[0].description}"`),
      ).toBeVisible();
      await expect(
        page.locator(`text="${testCommonAreas[0].hourlyRate}"`),
      ).toBeVisible();
    });
    ...
    await test.step("Consultar disponibilidad para fecha específica", async () => {
      ...
      const dateInput = page
        .locator('input[name="date"], input[type="date"], #reservation-date')
        .first();
      await dateInput.fill(testReservation.date);
      // Buscar disponibilidad
      const checkAvailabilityButton = page
        .locator(
          'button:has-text("Consultar"), button:has-text("Ver Disponibilidad")',
        )
        .first();
      ...
      await expect(
        page.locator("text=/disponible/i, .available, .time-slot"),
      ).toBeVisible();
    });
    ...
    await test.step("Crear nueva reserva", async () => {
      ...
      const reserveButton = page
        .locator(
          'button:has-text("Reservar"), button:has-text("Crear Reserva")',
        )
        .first();
      ...
      await expect(
        page.locator("form, .reservation-form, .modal"),
      ).toBeVisible();
      // Llenar detalles de la reserva
      await page.fill(
        'input[name="startTime"], input[type="time"], #start-time',
        testReservation.startTime,
      );
      await page.fill(
        'input[name="endTime"], input[type="time"], #end-time',
        testReservation.endTime,
      );
      await page.fill(
        'input[name="purpose"], #reservation-purpose',
        testReservation.purpose,
      );
      await page.fill(
        'input[name="guests"], #estimated-guests',
        testReservation.estimatedGuests,
      );
      await page.fill(
        'input[name="phone"], #contact-phone',
        testReservation.contactPhone,
      );
      ...
      for (const service of testReservation.additionalServices) {
        const serviceCheckbox = page
          .locator(
            `input[type="checkbox"][value*="${service}"], label:has-text("${service}")~input`,
          )
          .first();
      ...
      const specialRequestsField = page
        .locator('textarea[name="specialRequests"], #special-requests')
        .first();
      ...
      const totalCost =
        testCommonAreas[0].hourlyRate * testReservation.duration;
      await expect(
        page.locator(`text="${totalCost}", text="$${totalCost}"`),
      ).toBeVisible();
    });
    ...
    await test.step("Confirmar reserva y proceder al pago", async () => {
      ...
      const termsCheckbox = page
        .locator('input[type="checkbox"][name*="terms"], #accept-terms')
        .first();
      ...
      const confirmButton = page
        .locator(
          'button[type="submit"], button:has-text("Confirmar"), button:has-text("Continuar")',
        )
        .first();
      ...
      await page.waitForLoadState("networkidle");
      await expect(
        page.locator("text=/pago/i, text=/payment/i, .payment-form"),
    ...
    await test.step("Procesar pago de la reserva", async () => {
      ...
      await expect(
        page.locator(`text="${testReservation.area}"`),
      ).toBeVisible();
      await expect(
        page.locator(`text="${testReservation.date}"`),
      ).toBeVisible();
      await expect(
        page.locator(`text="${testReservation.startTime}"`),
      ).toBeVisible();
      // Seleccionar método de pago
      const paymentMethodSelect = page
        .locator('select[name="paymentMethod"], #payment-method')
        .first();
      ...
      const installmentsSelect = page
        .locator('select[name="installments"], #installments')
        .first();
      ...
      const payButton = page
        .locator(
          'button[type="submit"], button:has-text("Pagar"), button:has-text("Procesar")',
        )
        .first();
      ...
      await expect(
        page.locator(
          "text=/pago.*exitoso/i, text=/reserva.*confirmada/i, text=/transacción.*aprobada/i",
        ),
    ...
    await test.step("Verificar confirmación de reserva", async () => {
      ...
      await expect(
        page.locator("text=/confirmada/i, text=/aprobada/i"),
      ).toBeVisible();
      // Verificar número de confirmación o código de reserva
      const confirmationCode = page
        .locator('[data-testid="confirmation-code"], .confirmation-number')
        .first();
      ...
      const downloadReceiptButton = page
        .locator('button:has-text("Descargar"), a:has-text("Comprobante")')
        .first();
    ...
    await test.step("Verificar reserva en calendario personal", async () => {
      ...
      const myReservationsLink = page
        .locator(
          'a:has-text("Mis Reservas"), a:has-text("Mi Calendario"), [href*="my-reserv"]',
        )
        .first();
      ...
      await expect(
        page.locator("text=/confirmada/i, text=/activa/i"),
      ).toBeVisible();
    });
  });
  test("Validación de conflictos de horario", async ({ page }) => {
    ...
    if (await reserveButton.isVisible()) {
      ...
      await expect(
        page.locator(
          "text=/no.*disponible/i, text=/ocupado/i, text=/conflicto/i",
        ),
      ).toBeVisible();
    }
  });
  test("Cancelación de reserva por residente", async ({ page }) => {
    ...
    const cancelButton = page
      .locator('button:has-text("Cancelar"), .cancel-reservation')
      .first();
    if (await cancelButton.isVisible()) {
      ...
      const confirmCancelButton = page
        .locator(
          'button:has-text("Confirmar Cancelación"), button:has-text("Sí")',
        )
        .first();
      if (await confirmCancelButton.isVisible()) {
        ...
        await expect(
          page.locator("text=/cancelada/i, text=/anulada/i"),
        ).toBeVisible();
      }
    }
  });
  test("Gestión de reservas por administrador", async ({ page }) => {
    ...
    await page
      .locator('a:has-text("Reservas"), a:has-text("Gestión")')
      .first()
      .click();

    // Ver todas las reservas
    const allReservationsTab = page
      .locator('tab:has-text("Todas"), button:has-text("Ver Todas")')
      .first();
    ...
    await expect(
      page.locator(`text="${testUsers.resident1.name}"`),
    ).toBeVisible();
    await expect(page.locator(`text="${testReservation.area}"`)).toBeVisible();
    // Filtrar por área común
    const areaFilter = page
      .locator('select[name="area"], #area-filter')
      .first();
    ...
    const viewDetailsButton = page
      .locator('button:has-text("Ver"), button:has-text("Detalles")')
      .first();
    if (await viewDetailsButton.isVisible()) {
      ...
      await expect(
        page.locator(`text="${testReservation.purpose}"`),
      ).toBeVisible();
      await expect(
        page.locator(`text="${testReservation.estimatedGuests}"`),
      ).toBeVisible();
    }
  });
  test("Configuración de tarifas por administrador", async ({ page }) => {
    ...
    await page
      .locator('a:has-text("Configuración"), a:has-text("Áreas")')
      .first()
      .click();

    // Editar tarifa de un área
    const editAreaButton = page
      .locator(
        `text="${testCommonAreas[0].name}"~button:has-text("Editar"), .edit-area`,
      )
      .first();
    if (await editAreaButton.isVisible()) {
      ...
      const rateInput = page
        .locator('input[name="hourlyRate"], #hourly-rate')
        .first();
      if (await rateInput.isVisible()) {
        ...
        await expect(
          page.locator("text=/actualizada/i, text=/guardada/i"),
        ).toBeVisible();
      }
    }
  });
  test("Reporte de ocupación de áreas comunes", async ({ page }) => {
    ...
    // Navegar a reportes
    await page
      .locator('a:has-text("Reportes"), a:has-text("Estadísticas")')
      .first()
      .click();

    // Seleccionar reporte de reservas/ocupación
    const reservationReportTab = page
      .locator('tab:has-text("Reservas"), button:has-text("Ocupación")')
      .first();
    ...
    const periodSelect = page
      .locator('select[name="period"], #report-period')
      .first();
    ...
    const generateButton = page
      .locator('button:has-text("Generar"), button:has-text("Crear Reporte")')
      .first();
    if (await generateButton.isVisible()) {
      ...
      await expect(
        page.locator("text=/ocupación/i, .chart, .statistics"),
      ).toBeVisible();

      // Verificar métricas básicas
      await expect(
        page.locator("text=/total.*reservas/i, text=/ingresos/i"),
      ).toBeVisible();
    }
  });
});
