...
test.describe("Onboarding - Registro de Conjunto Residencial", () => {
  ...
  test("Flujo completo de registro de conjunto residencial", async ({
    page,
  }) => {
    ...
    await test.step("Completar información del conjunto", async () => {
      ...
      await page.fill(
        'input[name="name"], input[placeholder*="nombre"], #complex-name',
        testComplex.name,
      );
      await page.fill(
        'input[name="nit"], input[placeholder*="nit"], #complex-nit',
        testComplex.nit,
      );
      await page.fill(
        'input[name="address"], input[placeholder*="dirección"], #complex-address',
        testComplex.address,
      );
      await page.fill(
        'input[name="phone"], input[placeholder*="teléfono"], #complex-phone',
        testComplex.phone,
      );
      await page.fill(
        'input[name="email"], input[placeholder*="email"], #complex-email',
        testComplex.email,
      );

      // Información de estructura
      await page.fill(
        'input[name="units"], input[placeholder*="unidades"], #units',
        testComplex.units,
      );
      await page.fill(
        'input[name="towers"], input[placeholder*="torres"], #towers',
        testComplex.towers,
      );
      await page.fill(
        'input[name="floors"], input[placeholder*="pisos"], #floors',
        testComplex.floors,
      );
      ...
      const commonAreasField = page
        .locator(
          'textarea[name="commonAreas"], input[name="commonAreas"], #common-areas',
        )
        .first();
    ...
    await test.step("Configurar administrador principal", async () => {
      ...
      await page
        .locator("text=/administrador/i")
        .first()
        .scrollIntoViewIfNeeded();

      // Llenar datos del administrador
      await page.fill(
        'input[name="adminName"], input[placeholder*="nombre"], #admin-name',
        adminUser.name,
      );
      await page.fill(
        'input[name="adminEmail"], input[placeholder*="email"], #admin-email',
        adminUser.email,
      );
      await page.fill(
        'input[name="adminPhone"], input[placeholder*="teléfono"], #admin-phone',
        adminUser.phone,
      );
      await page.fill(
        'input[name="adminDni"], input[placeholder*="documento"], #admin-dni',
        adminUser.dni,
      );
      ...
      await page.fill(
        'input[name="confirmPassword"], input[placeholder*="confirmar"]',
    ...
    await test.step("Enviar formulario de registro", async () => {
      ...
      const submitButton = page
        .locator(
          'button[type="submit"], button:has-text("Registrar"), button:has-text("Crear")',
        )
        .first();
    ...
    await test.step("Verificar registro exitoso", async () => {
      ...
      await expect(
        page.locator(
          "text=/éxito/i, text=/exitoso/i, text=/registrado/i, text=/creado/i",
        ),
    ...
    await test.step("Verificar acceso del administrador", async () => {
      ...
      await expect(
        page.locator(
          "text=/dashboard/i, text=/panel/i, text=/administración/i",
        ),
      ).toBeVisible({ timeout: 10000 });

      // Verificar que aparece el nombre del conjunto o del usuario
      await expect(
        page.locator(`text="${testComplex.name}", text="${adminUser.name}"`),
      ).toBeVisible();
    });
  });
  test("Validación de campos requeridos en registro", async ({ page }) => {
    ...
    if (await submitButton.isVisible()) {
      ...
      await expect(
        page.locator(
          "text=/requerido/i, text=/obligatorio/i, .error, .invalid",
        ),
      ).toBeVisible();
    }
  });
  test("Validación de email duplicado", async ({ page }) => {
    ...
    await page.fill(
      'input[name="email"], #complex-email',
      "admin@existing-complex.com",
    );
    ...
    if (await submitButton.isVisible()) {
      await submitButton.click();
      await expect(
        page.locator("text=/email.*existe/i, text=/ya.*registrado/i"),
      ).toBeVisible({ timeout: 10000 });
    }
  });
});
