...
const testAssembly = {
  ...
  description:
    "Asamblea ordinaria para aprobación de presupuesto y elección de consejo de administración",
  date: "2025-03-15",
  time: "14:00",
...
const testProposals = [
  {
    ...
    description:
      "Aprobar el presupuesto anual para el año 2025 con un incremento del 8%",
  ...
  },
  {
    ...
    description:
      "Autorizar la inversión de $50,000,000 para renovación completa de la piscina",
    type: "INVESTMENT",
  ...
  },
  {
    ...
    description:
      "Elegir 3 miembros del consejo de administración para período 2025-2027",
    type: "ELECTION",
    options: [
      "Candidato A - María García",
      "Candidato B - Juan López",
      "Candidato C - Ana Martínez",
    ],
  },
];
test.describe("Asambleas - Gestión y Votación", () => {
  ...
  test("Flujo completo: Crear asamblea, votar y consultar resultados", async ({
    browser,
  }) => {
    ...
    await test.step("Admin crea nueva asamblea", async () => {
      ...
      const assemblyLink = adminPage
        .locator(
          'a:has-text("Asambleas"), a:has-text("Assemblies"), [href*="assembl"]',
        )
        .first();
      ...
      const createButton = adminPage
        .locator(
          'button:has-text("Crear"), button:has-text("Nueva"), button:has-text("Programar")',
        )
        .first();
      ...
      await adminPage.fill(
        'input[name="title"], #assembly-title',
        testAssembly.title,
      );
      await adminPage.fill(
        'textarea[name="description"], #assembly-description',
        testAssembly.description,
      );
      await adminPage.fill(
        'input[name="date"], input[type="date"], #assembly-date',
        testAssembly.date,
      );
      await adminPage.fill(
        'input[name="time"], input[type="time"], #assembly-time',
        testAssembly.time,
      );
      await adminPage.fill(
        'input[name="location"], #assembly-location',
        testAssembly.location,
      );
      await adminPage.fill(
        'input[name="quorum"], #assembly-quorum',
        testAssembly.quorum,
      );
      // Seleccionar tipo si existe
      const typeSelect = adminPage
        .locator('select[name="type"], #assembly-type')
        .first();
      ...
      await expect(
        adminPage.locator("text=/creada/i, text=/programada/i, text=/éxito/i"),
    ...
    await test.step("Configurar propuestas para votación", async () => {
      ...
      for (const proposal of testProposals) {
        const addProposalButton = adminPage
          .locator(
            'button:has-text("Agregar"), button:has-text("Nueva Propuesta")',
          )
          .first();
        if (await addProposalButton.isVisible()) {
          await addProposalButton.click();
          await adminPage.fill(
            'input[name="proposalTitle"], #proposal-title',
            proposal.title,
          );
          await adminPage.fill(
            'textarea[name="proposalDescription"], #proposal-description',
            proposal.description,
          );

          // Agregar opciones de votación
          for (let i = 0; i < proposal.options.length; i++) {
            const optionInput = adminPage
              .locator(`input[name="option${i}"], #option-${i}`)
              .first();
            if (await optionInput.isVisible()) {
              await optionInput.fill(proposal.options[i]);
            }
          }
          const saveProposalButton = adminPage
            .locator(
              'button:has-text("Guardar Propuesta"), button[type="submit"]',
            )
            .first();
    ...
    await test.step("Iniciar asamblea en vivo", async () => {
      ...
      await adminPage.locator(`text="${testAssembly.title}"`).click();
      // Iniciar asamblea
      const startButton = adminPage
        .locator('button:has-text("Iniciar"), button:has-text("Comenzar")')
        .first();
      if (await startButton.isVisible()) {
        await startButton.click();
        await expect(
          adminPage.locator("text=/iniciada/i, text=/activa/i, text=/vivo/i"),
        ).toBeVisible();
      }
      await adminPage.close();
    });
    ...
    await test.step("Residentes registran asistencia", async () => {
      ...
      for (const resident of residents) {
        ...
        const assemblyLink = residentPage
          .locator('a:has-text("Asamblea"), [href*="assembly"]')
          .first();
        ...
        const attendButton = residentPage
          .locator(
            'button:has-text("Confirmar Asistencia"), button:has-text("Presente")',
          )
          .first();
        if (await attendButton.isVisible()) {
          await attendButton.click();
          await expect(
            residentPage.locator(
              "text=/asistencia.*registrada/i, text=/presente/i",
            ),
          ).toBeVisible();
        }
      }
      ...
      await expect(
        adminPage.locator("text=/quórum/i, text=/alcanzado/i"),
    ...
    await test.step("Proceso de votación en tiempo real", async () => {
      ...
      const openVotingButton = adminPage
        .locator(
          'button:has-text("Abrir Votación"), button:has-text("Iniciar Voto")',
        )
        .first();
      if (await openVotingButton.isVisible()) {
        await openVotingButton.click();
        await expect(
          adminPage.locator("text=/votación.*abierta/i, text=/pueden.*votar/i"),
        ).toBeVisible();
      }
      // Residentes votan
      const residentPages = [];
      ...
      for (let i = 0; i < residents.length; i++) {
        ...
        // Emitir voto
        const voteOption = residentPage
          .locator(`button:has-text("${votes[i]}"), input[value="${votes[i]}"]`)
          .first();
        if (await voteOption.isVisible()) {
          await voteOption.click();

          const confirmVoteButton = residentPage
            .locator('button:has-text("Confirmar Voto"), button[type="submit"]')
            .first();
          ...
          await expect(
            residentPage.locator("text=/voto.*registrado/i, text=/gracias/i"),
          ).toBeVisible();
        }
      }
      // Admin cierra votación
      const closeVotingButton = adminPage
        .locator(
          'button:has-text("Cerrar Votación"), button:has-text("Finalizar")',
        )
        .first();
      if (await closeVotingButton.isVisible()) {
        await closeVotingButton.click();
        await expect(
          adminPage.locator("text=/votación.*cerrada/i, text=/resultados/i"),
        ).toBeVisible();
      }
      // Cerrar páginas
      for (const page of residentPages) {
    ...
    await test.step("Consultar resultados de votación", async () => {
      ...
      await expect(
        adminPage.locator('text="A favor": 2, text="En contra": 1'),
      ).toBeVisible();
      await expect(
        adminPage.locator("text=/aprobada/i, text=/ganadora/i"),
      ).toBeVisible();

      // Generar acta final
      const generateActaButton = adminPage
        .locator(
          'button:has-text("Generar Acta"), button:has-text("Finalizar")',
        )
        .first();
      if (await generateActaButton.isVisible()) {
        ...
        await expect(
          adminPage.locator(
            "text=/acta.*generada/i, text=/asamblea.*finalizada/i",
          ),
        ).toBeVisible();
      }
      await adminPage.close();
    ...
    await test.step("Verificar acceso a resultados históricos", async () => {
      ...
      const historyLink = residentPage
        .locator('a:has-text("Asambleas"), a:has-text("Histórico")')
        .first();
      if (await historyLink.isVisible()) {
        ...
        await expect(
          residentPage.locator(`text="${testAssembly.title}"`),
        ).toBeVisible();
        await expect(
          residentPage.locator("text=/finalizada/i, text=/completada/i"),
        ).toBeVisible();

        // Ver detalles de resultados
        await residentPage.locator(`text="${testAssembly.title}"`).click();
        await expect(
          residentPage.locator("text=/resultados/i, text=/acta/i"),
        ).toBeVisible();
      }
  ...
  test("Validación de quórum insuficiente", async ({ page }) => {
    ...
    const startVotingButton = page
      .locator('button:has-text("Iniciar Votación")')
      .first();
    if (await startVotingButton.isVisible()) {
      ...
      await expect(
        page.locator("text=/quórum.*insuficiente/i, text=/no.*alcanzado/i"),
      ).toBeVisible();
    }
  });
  test("Prevención de voto múltiple", async ({ browser }) => {
    ...
    const voteButton = residentPage
      .locator('button:has-text("A favor")')
      .first();
    if (await voteButton.isVisible()) {
      ...
      await expect(
        residentPage.locator("text=/ya.*votó/i, text=/voto.*registrado/i"),
      ).toBeVisible();
