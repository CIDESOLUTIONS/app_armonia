...
export class PlansController {
  ...
  @Post('subscribe')
  async subscribeToPlan(@GetUser() user: any, @Body() subscribeDto: SubscribeToPlanDto): Promise<SubscriptionDto> {
  ...
  async updateComplexPlan(@Param('complexId') complexId: string, @Body('newPlanType') newPlanType: PlanType) {
    return this.plansService.updateComplexPlan(+complexId, newPlanType);
  }

  @Get('complex/:complexId/subscription')
  async getComplexSubscription(@Param('complexId') complexId: string): Promise<SubscriptionDto | null> {
    return this.plansService.getComplexSubscription(+complexId);
  }

  @Get('check-feature/:featureName')
  async checkFeatureAccess(@GetUser() user: any, @Param('featureName') featureName: string): Promise<boolean> {
