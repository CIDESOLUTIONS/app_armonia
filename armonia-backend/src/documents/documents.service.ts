...
export class DocumentsService {
  ...
  async uploadDocument(
    ...
    const filePath = `documents/${schemaName}/${new Date().getFullYear()}/${String(
      new Date().getMonth() + 1,
    ).padStart(2, '0')}/${fileName}`;

    const autoType =
      createDocumentDto.type || this.categorizeByFileName(file.originalname);
    ...
    const document = await tenantPrisma.document.create({
      data: {
        ...
        accessRoles: (createDocumentDto.accessRoles as UserRole[]) || [],
  ...
  async uploadMultipleDocuments(
    ...
    const uploadPromises = files.map((file) =>
      this.uploadDocument(file, createDocumentDto, userId, schemaName),
    );
    ...
    const sharePromises = userIds.map((userId) =>
      tenantPrisma.documentShare.create({
        data: {
          ...
          recipientId: userId,
          shareToken: uuidv4(),
          accessLevel: permissions[0] || AccessLevel.VIEW,
  ...
  private buildWhereClause(
    ...
    if (searchDto.subcategory)
      where.subcategory = { contains: searchDto.subcategory, mode: 'insensitive' };
  ...
  private categorizeByFileName(fileName: string): DocumentType {
    ...
    if (lowerFileName.includes('reglamento')) return DocumentType.REGULATION;
    if (lowerFileName.includes('acta')) return DocumentType.MINUTES;
    if (lowerFileName.includes('manual')) return DocumentType.MANUAL;
    if (lowerFileName.includes('contrato')) return DocumentType.CONTRACT;
    if (lowerFileName.includes('factura')) return DocumentType.INVOICE;
    if (lowerFileName.includes('reporte')) return DocumentType.REPORT;
    if (lowerFileName.includes('certificado')) return DocumentType.CERTIFICATE;
    if (lowerFileName.includes('presupuesto')) return DocumentType.BUDGET;
  ...
  private mapToResponseDto(document: any): DocumentResponseDto {
    ...
    return {
      ...
      uploadedBy: uploadedBy
        ? { id: uploadedBy.id, name: uploadedBy.name, email: uploadedBy.email }
        : null,
      approvedBy: approvedBy
        ? { id: approvedBy.id, name: approvedBy.name, email: approvedBy.email }
        : null,
      residentialComplex: residentialComplex
        ? { id: residentialComplex.id, name: residentialComplex.name }
        : null,
      ...
      shares:
        shares?.map((s) => ({
          ...
          sharedBy: { id: s.sharedBy.id, name: s.sharedBy.name },
