...
export class CommunicationsController {
  ...
  async getUserNotifications(@GetUser() user: any, @Query() filters: any) {
    return this.communicationsService.getUserNotifications(user.userId, filters);
  }
  ...
  async confirmNotificationReading(@GetUser() user: any, @Param('id') id: string) {
    return this.communicationsService.confirmNotificationReading(id, user.userId);
  }

  // ANUNCIOS
  @Get('announcements')
  async getAnnouncements(@GetUser() user: any, @Query() filters: any) {
    return this.communicationsService.getAnnouncements(user.schemaName, user.userId, user.role, filters);
  }
  ...
  async getConversationMessages(@GetUser() user: any, @Param('id') conversationId: string, @Query() options: any) {
    return this.communicationsService.getConversationMessages(user.schemaName, conversationId, user.userId, options);
  }
  ...
  async markMessageAsRead(@GetUser() user: any, @Param('id') messageId: string) {
    return this.communicationsService.markMessageAsRead(user.schemaName, messageId, user.userId);
  }

  // EVENTOS COMUNITARIOS
  @Get('events')
  async getEvents(@GetUser() user: any, @Query() filters: any) {
    return this.communicationsService.getEvents(user.schemaName, user.userId, user.role, filters);
  }
