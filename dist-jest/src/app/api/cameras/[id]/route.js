    ...
    return __awaiter(this, arguments, void 0, function* (req, { params }) {
        try {
            ...
            if (!updateValidation.success) {
                return NextResponse.json({ error: 'Datos inválidos', details: updateValidation.error.format() }, { status: 400 });
            }
            // Inicializar Prisma
            const prisma = getPrisma();
            // Verificar que la cámara existe y pertenece al complejo
            const existingCamera = yield prisma.camera.findFirst({
                where: {
                    id,
                    complexId: payload.complexId
                }
            ...
            // Actualizar cámara
            const camera = yield prisma.camera.update({
                where: { id },
