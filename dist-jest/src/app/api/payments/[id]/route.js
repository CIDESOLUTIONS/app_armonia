...
import { UpdateTransactionSchema, TransactionIdSchema } from '@/validators/financial/payments.validator';
    ...
    return __awaiter(this, arguments, void 0, function* (request, { params }) {
        try {
            ...
            if (!['ADMIN', 'COMPLEX_ADMIN', 'RESIDENT'].includes(payload.role)) {
                return NextResponse.json({ message: 'Permisos insuficientes para acceder a transacciones' }, { status: 403 });
            }
            if (!payload.complexId) {
                return NextResponse.json({ message: 'Usuario no está asociado a un complejo residencial' }, { status: 400 });
            }
            // Validar parámetros
            ...
            if (!transaction) {
                return NextResponse.json({ message: 'Transacción no encontrada o sin permisos para acceder' }, { status: 404 });
            }
 ...
 */
function updateTransactionHandler(validatedData_1, request_1, _a) {
    return __awaiter(this, arguments, void 0, function* (validatedData, request, { params }) {
        try {
            ...
            if (!['ADMIN', 'COMPLEX_ADMIN'].includes(payload.role)) {
                return NextResponse.json({ message: 'Permisos insuficientes para actualizar transacciones' }, { status: 403 });
            }
            if (!payload.complexId) {
                return NextResponse.json({ message: 'Usuario no está asociado a un complejo residencial' }, { status: 400 });
            }
            ...
            if (!existingTransaction) {
                return NextResponse.json({ message: 'Transacción no encontrada en este complejo' }, { status: 404 });
            }
            // Verificar que la transacción se puede actualizar
            if (existingTransaction.status === 'COMPLETED' && validatedData.status !== 'REFUNDED') {
                return NextResponse.json({ message: 'No se puede modificar una transacción completada' }, { status: 400 });
            }
...
export const PUT = (request, context) => __awaiter(void 0, void 0, void 0, function* () {
...
export function DELETE(request_1, _a) {
    return __awaiter(this, arguments, void 0, function* (request, { params }) {
        try {
            ...
            if (!['ADMIN', 'COMPLEX_ADMIN', 'RESIDENT'].includes(payload.role)) {
                return NextResponse.json({ message: 'Permisos insuficientes para cancelar transacciones' }, { status: 403 });
            }
            if (!payload.complexId) {
                return NextResponse.json({ message: 'Usuario no está asociado a un complejo residencial' }, { status: 400 });
            }
            ...
            if (!['PENDING', 'PROCESSING'].includes(existingTransaction.status)) {
                return NextResponse.json({ message: 'Solo se pueden cancelar transacciones en estado PENDING o PROCESSING' }, { status: 400 });
            }
            // Cancelar transacción
            const cancelledTransaction = yield prisma.transaction.update({
                where: { id: transactionId },
                data: {
                    status: 'CANCELLED',
                    updatedAt: new Date(),
