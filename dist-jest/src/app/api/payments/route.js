...
import { GetTransactionsSchema, CreateTransactionSchema } from '@/validators/financial/payments.validator';
 ...
 */
function createTransactionHandler(validatedData, request) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Verificar autenticación
            const { auth, payload } = yield verifyAuth(request);
            ...
            if (!['ADMIN', 'COMPLEX_ADMIN', 'RESIDENT'].includes(payload.role)) {
                return NextResponse.json({ message: 'Permisos insuficientes para crear transacciones' }, { status: 403 });
            }
            if (!payload.complexId) {
                return NextResponse.json({ message: 'Usuario no está asociado a un complejo residencial' }, { status: 400 });
            }
            ...
            const paymentMethod = yield prisma.paymentMethod.findFirst({
                where: {
            ...
            if (!paymentMethod) {
                return NextResponse.json({ message: 'Método de pago no encontrado en este complejo' }, { status: 404 });
            }
            // Si hay invoiceId, verificar que pertenece al complejo y usuario
            if (validatedData.invoiceId) {
                const invoice = yield prisma.invoice.findFirst({
                    where: Object.assign({ id: validatedData.invoiceId, complexId: payload.complexId }, (payload.role === 'RESIDENT' && { userId: payload.userId }))
                });
                if (!invoice) {
                    return NextResponse.json({ message: 'Factura no encontrada o sin permisos para acceder' }, { status: 404 });
                }
                // Verificar que la factura no esté ya pagada
                if (invoice.status === 'PAID') {
                    return NextResponse.json({ message: 'La factura ya ha sido pagada' }, { status: 400 });
                }
            }
            ...
            const result = yield prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                const transaction = yield tx.transaction.create({
                    data: {
                        ...
                        status: 'PENDING',
                        metadata: Object.assign(Object.assign({}, validatedData.metadata), { ipAddress,
                            userAgent, createdBy: payload.email })
                    },
                    include: {
                        invoice: {
                            select: {
            ...
            if (error.code === 'P2002') {
                return NextResponse.json({ message: 'Error de duplicación de datos' }, { status: 409 });
            }
            return NextResponse.json({ message: 'Error interno del servidor' }, { status: 500 });
        }
    });
