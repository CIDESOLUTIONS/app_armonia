...
import { SendNotificationSchema } from '@/validators/notifications/notification.validator';
/**
 * POST /api/notifications/send
 * Enviar notificación push
 */
function sendNotificationHandler(validatedData, request) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const { auth, payload: authPayload } = yield verifyAuth(request);
            if (!auth || !authPayload) {
            ...
            // Enviar notificación
            const result = yield pushNotificationService.sendNotification(notificationRequest);
            if (!result.success) {
                return NextResponse.json({
