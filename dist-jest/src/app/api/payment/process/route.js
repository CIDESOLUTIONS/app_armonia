...
export function POST(_req) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const { planCode, cardNumber, expiryDate, cvv, cardholderName, amount, currency = 'COP', complexData // Datos del conjunto si aún no está creado
             } = body;
            ...
            try {
                yield prisma.$executeRawUnsafe(`CREATE SCHEMA IF NOT EXISTS "armonia"`);
            }
            catch (err) {
      ...
      `);
          ...
          `);
                        ...
                        let complexCount = 0;
                        try {
                            const complexCountResult = yield prisma.$queryRawUnsafe(`SELECT COUNT(*) as count FROM "armonia"."ResidentialComplex"`);
                        ...
                        const tempComplex = yield prisma.$queryRawUnsafe(`INSERT INTO "armonia"."ResidentialComplex" (
              name, "schemaName", "totalUnits", "adminEmail", "adminName", 
              "planCode", "planStatus", "createdAt", "updatedAt"
            ) VALUES ($1, $2, $3, $4, $5, $6, 'PENDING_PAYMENT', NOW(), NOW()) RETURNING id`, complexName, tempSchemaName, totalUnits, adminEmail, adminName, planCode);
            ...
            try {
                ...
                const paymentResult = yield prisma.$queryRawUnsafe(`INSERT INTO "armonia"."PaymentTransaction" (
          "complexId", "planCode", amount, currency, status, 
          "paymentMethod", "transactionId", "paymentGateway", "gatewayResponse", "expiresAt"
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, NOW() + INTERVAL '30 minutes') RETURNING id`, complexId, planCode, amount, currency, gatewayResponseSuccess ? 'COMPLETED' : 'FAILED', 'CREDIT_CARD', transactionId, 'SIMULATION', JSON.stringify(gatewayResponse) // Convertimos el objeto a string JSON y lo enviamos como JSONB
                );
                ...
                if (gatewayResponseSuccess && complexId) {
                    try {
                        yield prisma.$queryRawUnsafe(`UPDATE "armonia"."ResidentialComplex" 
             SET "planStatus" = 'ACTIVE', "lastPaymentId" = $1, "trialEndsAt" = NOW() + INTERVAL '30 days'
             WHERE id = $2`, paymentResult[0].id, complexId);
