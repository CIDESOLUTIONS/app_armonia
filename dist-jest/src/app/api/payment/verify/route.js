...
export function GET(_req) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      `);
            ...
            try {
                const transaction = yield prisma.$queryRawUnsafe(`SELECT * FROM "armonia"."PaymentTransaction" WHERE "transactionId" = $1`, transactionId);
                ...
                if (paymentTransaction.status === 'PENDING' && new Date(paymentTransaction.expiresAt) < new Date()) {
                    try {
                        yield prisma.$queryRawUnsafe(`UPDATE "armonia"."PaymentTransaction" SET status = 'EXPIRED', "updatedAt" = NOW() WHERE id = $1`, paymentTransaction.id);
                    }
                    catch (err) {
                ...
                if (paymentTransaction.complexId) {
                    try {
                        const complexResult = yield prisma.$queryRawUnsafe(`SELECT id, name, "planCode", "planStatus" FROM "armonia"."ResidentialComplex" WHERE id = $1`, paymentTransaction.complexId);
