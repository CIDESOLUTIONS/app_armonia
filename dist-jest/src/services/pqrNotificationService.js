...
export class PQRNotificationService {
     ...
     */
    notifyStatusChange(pqrId, newStatus, previousStatus, changedById, comment) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const reporter = yield this.prisma.user.findUnique({
                    where: { id: pqr.userId }
                });
                // Obtener información del usuario asignado (si existe)
                let assignee = null;
                if (pqr.assignedToId) {
                    assignee = yield this.prisma.user.findUnique({
                        where: { id: pqr.assignedToId }
                    });
                }
                // Obtener plantilla de notificación
                const template = yield this.getNotificationTemplate(newStatus, previousStatus);
                ...
                if (reporter) {
                    ...
                    const reporterData = Object.assign(Object.assign({}, templateData), { recipientName: reporter.name });
                    // Enviar email
                    yield sendEmail({
                        to: reporter.email,
                    ...
                    yield this.prisma.pQRNotification.create({
                        data: {
                            pqrId,
                            userId: reporter.id,
                            type: 'STATUS_CHANGE',
                ...
                if (assignee) {
                    ...
                    const assigneeData = Object.assign(Object.assign({}, templateData), { recipientName: assignee.name });
                    // Enviar email
                    yield sendEmail({
                        to: assignee.email,
     ...
     */
    getNotificationTemplate(newStatus, previousStatus) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const customTemplate = yield this.prisma.notificationTemplate.findFirst({
                    where: {
                        schemaName: this.schema,
                        name: templateName,
                    },
    ...
    replaceTemplateVars(template, data) {
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            result = result.replace(regex, (value === null || value === void 0 ? void 0 : value.toString()) || '');
        }
     ...
     */
    generateSurveyToken(pqrId, userId) {
