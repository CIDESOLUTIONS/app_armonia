...
export class PQRAssignmentService {
    ...
    processPQR(pqrData) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const categorization = yield this.categorize(pqrData);
                // 2. Determinar la prioridad
                const prioritization = yield this.prioritize(pqrData, categorization.category);
                // 3. Asignar a equipo o persona responsable
                const assignment = yield this.assign(pqrData, categorization.category, prioritization.priority);
                // 4. Calcular fecha límite según SLA
                const dueDate = yield this.calculateDueDate(categorization.category, prioritization.priority);
                // 5. Generar etiquetas para clasificación
                const tags = yield this.generateTags(pqrData, categorization.category);
                // Combinar resultados
                return Object.assign(Object.assign(Object.assign(Object.assign({}, categorization), prioritization), assignment), { dueDate,
                    tags });
            }
            catch (error) {
    ...
    categorize(pqrData) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const settings = yield this.prisma.pQRSettings.findFirst();
                ...
                const rules = yield this.prisma.pQRAssignmentRule.findMany({
                    where: { isActive: true },
                ...
                for (const rule of rules) {
                    if (rule.keywords && rule.keywords.length > 0) {
                        ...
                        const matchesKeyword = rule.keywords.some(keyword => combinedText.includes(keyword.toLowerCase()));
     ...
     */
    determineSubcategory(text, category) {
        ...
        if (category === PQRCategory.MAINTENANCE) {
            ...
            if (text.match(/luz|el[eé]ctric|bombill|enchufe|corto|apag[oó]n/i))
                return 'Eléctrico';
            if (text.match(/ascensor|elevador/i))
                return 'Ascensores';
            if (text.match(/puerta|cerradura|llave|chapa/i))
                return 'Cerrajería';
            if (text.match(/pared|muro|grieta|fisura|pintura/i))
                return 'Estructural';
        }
        // Subcategorías para seguridad
        else if (category === PQRCategory.SECURITY) {
            if (text.match(/c[aá]mara|cctv|video/i))
                return 'Videovigilancia';
            if (text.match(/alarm|sensor/i))
                return 'Alarmas';
            if (text.match(/robo|hurto|intrusi[oó]n/i))
                return 'Incidentes';
            if (text.match(/vigilan|guardia|portero/i))
                return 'Personal';
        }
        // Subcategorías para servicios
        else if (category === PQRCategory.SERVICES) {
            if (text.match(/internet|wifi|red/i))
                return 'Internet';
            if (text.match(/gimnasio|gym/i))
                return 'Gimnasio';
            if (text.match(/piscina|alberca|jacuzzi/i))
                return 'Piscina';
            if (text.match(/sal[oó]n|evento|fiesta/i))
                return 'Salón comunal';
            if (text.match(/parque|juego|infantil/i))
                return 'Zonas infantiles';
        }
        return undefined;
    }
    /**
     ...
     */
    assign(pqrData, category, priority) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (rules && rules.length > 0) {
                    ...
                    if (rule.assignToTeamId) {
                        const team = yield this.prisma.pQRTeam.findUnique({
                ...
                const teams = yield this.prisma.pQRTeam.findMany({
                    where: {
                        isActive: true,
                        categories: { has: category },
                    },
     ...
     */
    calculateDueDate(category, priority) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Buscar SLA específico para esta categoría y prioridad
                const sla = yield this.prisma.pQRSLA.findFirst({
                    where: {
                        isActive: true,
                        OR: [
                            { category },
