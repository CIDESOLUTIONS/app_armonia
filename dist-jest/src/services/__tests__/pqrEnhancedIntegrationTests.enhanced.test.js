...
import { PQRStatus, PQRCategory, PQRPriority } from '@prisma/client';
...
describe('Sistema PQR - Pruebas de Integración Avanzadas', () => {
    ...
    beforeEach(() => {
        ...
        prisma = assignmentService.prisma;
    });
    describe('Flujo completo de PQR', () => {
        it('debe procesar un PQR desde creación hasta resolución', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const createdPQR = Object.assign(Object.assign({ id: 123, ticketNumber: 'PQR-20250602-001' }, pqrData), { status: PQRStatus.OPEN, submittedAt: new Date(), dueDate: null });
            prisma.pQR.create.mockResolvedValueOnce(createdPQR);
            // 6. Procesar PQR (categorización y asignación)
            const processResult = yield assignmentService.processPQR(pqrData);
            ...
            const categorizedPQR = Object.assign(Object.assign({}, createdPQR), { id: 123, status: PQRStatus.CATEGORIZED, category: processResult.category, subcategory: processResult.subcategory, priority: processResult.priority, assignedTeamId: processResult.assignedTeamId, assignedTeamName: 'Equipo Plomería' });
            ...
            const notifyResult1 = yield notificationService.notifyStatusChange(123, PQRStatus.CATEGORIZED, PQRStatus.OPEN, null, 'PQR categorizado automáticamente');
            ...
            const assignedPQR = Object.assign(Object.assign({}, categorizedPQR), { status: PQRStatus.ASSIGNED, assignedToId: 10, assignedToName: 'Técnico Plomería', assignedToRole: 'STAFF', assignedAt: new Date() });
            ...
            const notifyResult2 = yield notificationService.notifyStatusChange(123, PQRStatus.ASSIGNED, PQRStatus.CATEGORIZED, 10, 'PQR asignado a técnico');
            // Verificar notificación
            ...
            const inProgressPQR = Object.assign(Object.assign({}, assignedPQR), { status: PQRStatus.IN_PROGRESS, startedAt: new Date() });
            ...
            const notifyResult3 = yield notificationService.notifyStatusChange(123, PQRStatus.IN_PROGRESS, PQRStatus.ASSIGNED, 10, 'Iniciando trabajo en la solicitud');
            // Verificar notificación
            expect(notifyResult3).toBe(true);
            // 18. Mock para resolución de PQR
            const resolvedPQR = Object.assign(Object.assign({}, inProgressPQR), { status: PQRStatus.RESOLVED, resolvedAt: new Date(), resolution: 'Se reparó la fuga de agua y se verificó su correcto funcionamiento' });
            ...
            const notifyResult4 = yield notificationService.notifyStatusChange(123, PQRStatus.RESOLVED, PQRStatus.IN_PROGRESS, 10, 'PQR resuelto satisfactoriamente');
            // Verificar notificación
            ...
            const surveyResult = yield notificationService.sendSatisfactionSurvey(123);
            ...
            const closedPQR = Object.assign(Object.assign({}, resolvedPQR), { status: PQRStatus.CLOSED, closedAt: new Date(), satisfactionRating: 5, satisfactionComment: 'Excelente servicio, rápido y efectivo' });
            ...
            prisma.$queryRaw.mockResolvedValueOnce([{ autoNotifyEnabled: true }]);
            // 25. Notificar cierre
            const notifyResult5 = yield notificationService.notifyStatusChange(123, PQRStatus.CLOSED, PQRStatus.RESOLVED, 1, 'PQR cerrado por el usuario');
            // Verificar notificación
            ...
            prisma.pQR.findMany.mockResolvedValueOnce([closedPQR]);
            // 27. Generar métricas
            const metrics = yield metricsService.getSummaryMetrics();
            // Verificar métricas
    ...
    describe('Escenarios de integración avanzados', () => {
        it('debe manejar correctamente la reapertura y escalamiento de un PQR', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const reopenedPQR = Object.assign(Object.assign({}, resolvedPQR), { status: PQRStatus.REOPENED, reopenReason: 'La cerradura sigue fallando después de la reparación', reopenCount: 1, reopenedAt: new Date() });
            ...
            const notifyResult1 = yield notificationService.notifyStatusChange(124, PQRStatus.REOPENED, PQRStatus.RESOLVED, 2, 'PQR reabierto por el usuario');
            // Verificar notificación
            expect(notifyResult1).toBe(true);
            expect(sendEmail).toHaveBeenCalledWith(expect.objectContaining({
            ...
            const processResult = yield assignmentService.processPQR(Object.assign(Object.assign({}, reopenedPQR), { previousAssignedToId: 11 }));
            // Verificar resultado de procesamiento
            expect(processResult.category).toBe(PQRCategory.MAINTENANCE);
            ...
            const escalatedPQR = Object.assign(Object.assign({}, reopenedPQR), { status: PQRStatus.ASSIGNED, priority: processResult.priority, assignedToId: processResult.assignedToId, assignedToName: processResult.assignedToName, assignedToRole: processResult.assignedToRole, escalated: true, escalatedAt: new Date(), escalationReason: 'Reapertura de solicitud' });
            ...
            const notifyResult2 = yield notificationService.notifyStatusChange(124, PQRStatus.ASSIGNED, PQRStatus.REOPENED, 15, 'PQR escalado y reasignado a especialista');
            // Verificar notificación
            expect(notifyResult2).toBe(true);
            expect(sendEmail).toHaveBeenCalledWith(expect.objectContaining({
            ...
            const escalationRate = yield metricsService.getEscalationRate();
            expect(escalationRate).toBe(100); // 100% de escalamiento
        }));
        it('debe generar correctamente métricas de SLA y rendimiento', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const summaryMetrics = yield metricsService.getSummaryMetrics();
            ...
            const slaMetrics = yield metricsService.getSLAMetrics();
            ...
            const categoryDistribution = yield metricsService.getCategoryDistribution();
            ...
            const priorityDistribution = yield metricsService.getPriorityDistribution();
            ...
            const timeTrend = yield metricsService.getTimeTrend();
            ...
            const dashboard = yield metricsService.generateDashboardMetrics();
            // Verificar dashboard
    ...
    describe('Manejo de errores en integración', () => {
        it('debe manejar errores durante el flujo completo sin interrumpir el proceso', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. Datos de entrada para nuevo PQR
            const pqrData = {
                type: 'COMPLAINT',
            ...
            const createdPQR = Object.assign(Object.assign({ id: 125, ticketNumber: 'PQR-20250602-003' }, pqrData), { status: PQRStatus.OPEN, category: PQRCategory.OTHER, priority: PQRPriority.MEDIUM, submittedAt: new Date() });
            prisma.pQR.create.mockResolvedValueOnce(createdPQR);
            ...
            const notifyResult = yield notificationService.notifyStatusChange(125, PQRStatus.OPEN, null, null, 'Nuevo PQR creado');
            // Verificar resultado
            expect(notifyResult).toBe(false);
            // 7. Mock para actualización manual de categoría
            const categorizedPQR = Object.assign(Object.assign({}, createdPQR), { status: PQRStatus.CATEGORIZED, category: PQRCategory.MAINTENANCE, subcategory: 'Eléctrico', priority: PQRPriority.MEDIUM });
            prisma.pQR.update.mockResolvedValueOnce(categorizedPQR);
            prisma.pQR.findUnique.mockResolvedValueOnce(categorizedPQR);
            // 8. Mock para configuración de notificaciones
            prisma.$queryRaw.mockResolvedValueOnce([{ autoNotifyEnabled: true }]);
            ...
            const notifyResult2 = yield notificationService.notifyStatusChange(125, PQRStatus.CATEGORIZED, PQRStatus.OPEN, null, 'PQR categorizado manualmente');
            // Verificar notificación
            expect(notifyResult2).toBe(true);
