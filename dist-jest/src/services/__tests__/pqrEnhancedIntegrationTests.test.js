...
import { PQRCategory, PQRPriority, PQRStatus } from '@prisma/client';
...
describe('PQR System Integration Tests - Flujos Principales', () => {
    ...
    describe('Flujo completo de PQR con asignación inteligente', () => {
        it('debe procesar un PQR desde creación hasta resolución con asignación inteligente', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const assignmentResult = yield assignmentService.processPQR(pqrData);
            ...
            const notificationResult = yield notificationService.notifyStatusChange(mockPQR.id, PQRStatus.OPEN, undefined, 1, // userId
            ...
            const updatedPQR = Object.assign(Object.assign({}, mockPQR), { status: PQRStatus.ASSIGNED, assignedToId: 10, assignedToName: 'Técnico 1', assignedToRole: 'STAFF', assignedAt: new Date() });
            ...
            const assignNotificationResult = yield notificationService.notifyStatusChange(mockPQR.id, PQRStatus.ASSIGNED, PQRStatus.OPEN, 5, // adminId
            'Asignado al técnico 1');
            // Verificar resultado de notificación
            expect(assignNotificationResult).toBe(true);
            // 9. Simular cambio a estado en progreso
            const inProgressPQR = Object.assign(Object.assign({}, updatedPQR), { status: PQRStatus.IN_PROGRESS });
            ...
            const inProgressNotificationResult = yield notificationService.notifyStatusChange(mockPQR.id, PQRStatus.IN_PROGRESS, PQRStatus.ASSIGNED, 10, // técnicoId
            ...
            const resolvedNotificationResult = yield notificationService.notifyStatusChange(mockPQR.id, PQRStatus.RESOLVED, PQRStatus.IN_PROGRESS, 10, // técnicoId
            ...
            const surveyResult = yield notificationService.sendSatisfactionSurvey(mockPQR.id);
            // Verificar resultado de encuesta
            expect(surveyResult).toBe(true);
            // 15. Simular calificación y cierre del PQR
            const closedPQR = Object.assign(Object.assign({}, resolvedPQR), { status: PQRStatus.CLOSED, satisfactionRating: 5, satisfactionComment: 'Excelente servicio, muy rápido', closedAt: new Date() });
            ...
            const closedNotificationResult = yield notificationService.notifyStatusChange(mockPQR.id, PQRStatus.CLOSED, PQRStatus.RESOLVED, 1, // userId
            ...
            const metricsResult = yield metricsService.getSummaryMetrics();
    ...
    describe('Flujo de PQR con reapertura y escalamiento', () => {
        it('debe manejar correctamente la reapertura y escalamiento de un PQR', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const reopenedPQR = Object.assign(Object.assign({}, resolvedPQR), { status: PQRStatus.REOPENED, reopenReason: 'El problema persiste, sigue habiendo cortes de energía', reopenedAt: new Date() });
            ...
            const reopenNotificationResult = yield notificationService.notifyStatusChange(reopenedPQR.id, PQRStatus.REOPENED, PQRStatus.RESOLVED, 2, // userId
            ...
            const escalationResult = yield assignmentService.processPQR(Object.assign(Object.assign({}, reopenedPQR), { description: reopenedPQR.description + ' ' + reopenedPQR.reopenReason, escalated: true }));
            ...
            const escalatedPQR = Object.assign(Object.assign({}, reopenedPQR), { status: PQRStatus.ASSIGNED, priority: PQRPriority.HIGH, assignedTeamId: 3, assignedToId: null, assignedToName: null, assignedAt: new Date() });
            ...
            const escalationNotificationResult = yield notificationService.notifyStatusChange(escalatedPQR.id, PQRStatus.ASSIGNED, PQRStatus.REOPENED, 5, // adminId
            ...
            const assignedEscalatedPQR = Object.assign(Object.assign({}, escalatedPQR), { assignedToId: 12, assignedToName: 'Especialista 1', assignedToRole: 'SPECIALIST' });
            ...
            const specialistAssignNotificationResult = yield notificationService.notifyStatusChange(assignedEscalatedPQR.id, PQRStatus.ASSIGNED, PQRStatus.ASSIGNED, 5, // adminId
            'Asignado a especialista eléctrico');
            // Verificar resultado de notificación
            expect(specialistAssignNotificationResult).toBe(true);
            // 11. Simular resolución final del PQR
            const finalResolvedPQR = Object.assign(Object.assign({}, assignedEscalatedPQR), { status: PQRStatus.RESOLVED, resolvedAt: new Date(), resolution: 'Se identificó y reparó un problema en el tablero principal que causaba los cortes intermitentes' });
            ...
            const finalResolvedNotificationResult = yield notificationService.notifyStatusChange(finalResolvedPQR.id, PQRStatus.RESOLVED, PQRStatus.ASSIGNED, 12, // especialistaId
            ...
            prisma.$queryRaw.mockResolvedValueOnce([{ satisfactionSurveyEnabled: true }]);
            // 14. Enviar encuesta de satisfacción
            const surveyResult = yield notificationService.sendSatisfactionSurvey(finalResolvedPQR.id);
            // Verificar resultado de encuesta
            expect(surveyResult).toBe(true);
    ...
    describe('Flujo de recordatorios y alertas de SLA', () => {
        it('debe enviar recordatorios para PQRs próximos a vencer y generar alertas de SLA', () => __awaiter(void 0, void 0, void 0, function* () {
            // Fecha actual
            const now = new Date();
            ...
            const reminderCount = yield notificationService.sendDueDateReminders();
            // Verificar resultado
            ...
            const recentActivity = yield metricsService.getRecentActivity();
            // Verificar resultado
    ...
    describe('Generación de dashboard completo', () => {
        it('debe generar dashboard completo con todos los indicadores', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const dashboardMetrics = yield metricsService.generateDashboardMetrics();
            // Verificar resultado
            ...
            yield metricsService.generateDashboardMetrics(categoryFilter);
            ...
            yield metricsService.generateDashboardMetrics(dateFilter);
