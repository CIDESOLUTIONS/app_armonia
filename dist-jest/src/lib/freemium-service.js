...
export class FreemiumService {
     ...
     */
    static hasFeatureAccess(planType, feature) {
     ...
     */
    static validateUnitsLimit(planType, currentUnits, trialActive = false) {
        const plan = PLAN_FEATURES[planType];
     ...
     */
    static calculateMonthlyCost(planType, totalUnits) {
        const plan = PLAN_FEATURES[planType];
        if (planType === 'BASIC')
            return 0;
     ...
     */
    static hasHistoryAccess(planType, dataDate) {
        const plan = PLAN_FEATURES[planType];
     ...
     */
    static getMissingFeatures(currentPlan, targetPlan) {
     ...
     */
    static isTrialActive(trialEndDate) {
        if (!trialEndDate)
            return false;
     ...
     */
    static isSubscriptionExpired(planEndDate, planType) {
        if (planType === 'BASIC')
            return false; // Plan básico no expira
        if (!planEndDate)
            return false;
