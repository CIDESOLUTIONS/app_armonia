...
function generateKey(password, salt) {
    try {
        ...
        const derivedKey = crypto.scryptSync(password, salt || process.env.ENCRYPTION_SALT || 'armonia-salt', KEY_LENGTH);
        return derivedKey;
    }
    catch (error) {
...
export function encryptData(data, password) {
    try {
        ...
        const key = generateKey(password || process.env.ENCRYPTION_KEY || 'armonia-encryption-key', process.env.ENCRYPTION_SALT);
