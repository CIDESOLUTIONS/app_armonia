 ...
 */
export const DEFAULT_LANGUAGE = LANGUAGE_CODES.find((code) => SUPPORTED_LANGUAGES[code].isDefault) || 'es';
/**
 ...
 * @returns {LanguageConfig | null} Configuración del idioma o null si no existe
 */
 ...
 * @returns {Array<LanguageConfig>} Lista de configuraciones de idiomas
 */
 ...
 */
export function formatDate(date, langCode = DEFAULT_LANGUAGE) {
    ...
    return new Intl.DateTimeFormat(`${language.code}-${language.countryCode}`, options).format(date);
}
/**
 ...
 */
export function formatTime(date, langCode = DEFAULT_LANGUAGE) {
    const language = SUPPORTED_LANGUAGES[langCode] || SUPPORTED_LANGUAGES[DEFAULT_LANGUAGE];
    const options = {
        hour: '2-digit',
        minute: '2-digit',
 ...
 * @param {Intl.NumberFormatOptions} options - Opciones de formato
 * @param {string} langCode - Código de idioma
 * @returns {string} Número formateado
 */
export function formatNumber(num, options = {}, langCode = DEFAULT_LANGUAGE) {
    const language = SUPPORTED_LANGUAGES[langCode] || SUPPORTED_LANGUAGES[DEFAULT_LANGUAGE];
    return new Intl.NumberFormat(`${language.code}-${language.countryCode}`, options).format(num);
}
/**
 ...
 */
export function formatCurrency(amount, currencyCode = 'COP', langCode = DEFAULT_LANGUAGE) {
    const language = SUPPORTED_LANGUAGES[langCode] || SUPPORTED_LANGUAGES[DEFAULT_LANGUAGE];
    const options = {
        style: 'currency',
    ...
    return new Intl.NumberFormat(`${language.code}-${language.countryCode}`, options).format(amount);
}
