...
export function POST(_req) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const users = yield prisma.$queryRawUnsafe(`
      SELECT id, email, name, password, role, "complexId" 
      FROM "armonia"."User" 
      WHERE email = $1 AND active = true
    `, email);
            ServerLogger.debug(`Resultado de búsqueda de usuario: ${(users === null || users === void 0 ? void 0 : users.length) || 0} usuarios encontrados`);
            ...
            if (!user.complexId) {
                ...
                const _token = yield generateToken(payload);
            ...
            try {
                const complex = yield prisma.$queryRawUnsafe(`
      ...
      `, user.complexId);
                if (!complex || complex.length === 0) {
                    ...
                    return NextResponse.json({ message: "Error en la información del conjunto residencial" }, { status: 500 });
                }
