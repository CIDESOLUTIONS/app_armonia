...
export async function POST(_req: unknown) {
  try {
    ...
    ServerLogger.debug(
      `Resultado de búsqueda de usuario: ${users?.length || 0} usuarios encontrados`,
    );

    if (!users || users.length === 0) {
      ServerLogger.warn(
        `Login fallido para ${email}: Usuario no encontrado o inactivo`,
      );
    ...
    try {
      ...
      if (!complex || complex.length === 0) {
        ServerLogger.error(
          `Login fallido para ${email}: Conjunto residencial no encontrado (ID: ${user.complexId})`,
        );
        return NextResponse.json(
      ...
      ServerLogger.error(
        `Error al obtener información del conjunto residencial:`,
        error,
      );
      return NextResponse.json(
        { message: "Error en la información del conjunto residencial" },
